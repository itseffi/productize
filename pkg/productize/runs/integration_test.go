				...
				Events: []events.Event{
					{
						...
						Payload: []byte(
							`{"workspace_root":"/workspace","artifacts_dir":"/tmp/home/.productize/runs/run-2","ide":"codex","model":"gpt-5.5"}`,
						),
					},
				},
			},
		...
		events: map[string]apicore.RunEventPage{
			"run-compare": {
				Events: []events.Event{{
					...
					Kind:          events.EventKindRunStarted,
					Payload: []byte(
						`{"workspace_root":"/workspace","artifacts_dir":"/tmp/home/.productize/runs/run-compare","ide":"codex","model":"gpt-5.5"}`,
					),
				}},
			},
		},
