...
func TestApplySummaryEventDetailsUsesRunAndJobPayloads(t *testing.T) {
	...
	summary := RunSummary{
		...
		ArtifactsDir: "/home/example/.productize/runs/run-123",
	}
	applySummaryEventDetails(&summary, []events.Event{
		{
			...
			Payload: []byte(
				`{"workspace_root":"/workspace","artifacts_dir":"/home/example/.productize/runs/run-123","ide":"codex","model":"gpt-5.5"}`,
			),
		},
		{
			SchemaVersion: events.SchemaVersion,
			...
			Payload:       []byte(`{"artifacts_dir":"/home/example/.productize/runs/run-123","error":"boom"}`),
		},
	})
