...
- local enablement with `productize ext enable`
...
5. Replace recursive `productize` shellouts with `host.runs.start`.
...
| Writing `.productize/tasks/...` files directly              | Use `host.tasks.create` or `host.memory.write`                                        |
| Shelling out to `productize exec` from inside the extension | Use `host.runs.start` so recursion protection and parent-run propagation stay correct |
| Implicit trust by repository presence                    | Install and enable explicitly with `productize ext install` and `productize ext enable`     |
| Template-specific internal fixtures                      | Start from `@productize/create-extension` and keep local tests on the official harness   |
