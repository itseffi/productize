...
The Host API is the extension -> Productize callback surface available after a successful `initialize` response.
...
- Every call is recorded in `~/.productize/runs/<run-id>/run.db` (`hook_runs`).
...
- Paths must stay under the workspace root or `.productize/`.
...
- Memory documents are Markdown files under `.productize/tasks/<workflow>/memory/`.
- Omitting `task_file` targets `MEMORY.md`.
- `mode: "append"` appends with a newline separator.

## Review providers

Extensions can register executable review providers that Productize calls during `reviews fetch` and `reviews fix` workflows. The host dispatches `fetch_reviews` and `resolve_issues` JSON-RPC calls to the extension subprocess.
