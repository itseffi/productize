...
| `review.mutate`      | All `review.*` hooks                                                 | yes                                                    | Used by `productize reviews fix` and the daemon-owned `productize reviews watch` parent run.                                  |
| `artifacts.read`     | `host.artifacts.read`                                                | yes                                                    | Scoped to the workspace root and `.productize/`.                                                                           |
...
| `agents.ship`        | `[resources] agents = [...]` manifest declarations                   | validated at discovery/install                         | Used by declarative reusable-agent packs.                                                                               |
