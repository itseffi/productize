...
All hook payload and patch interfaces are exported from `@productize/extension-sdk`.
...
All `review.*` hooks require `review.mutate`. The fetch/fix hooks are active under `productize reviews fix`;
the watch hooks are active under the daemon-owned `productize reviews watch` parent run.
