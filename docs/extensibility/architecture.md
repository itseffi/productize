...
Productize extension execution is intentionally simple: every executable extension is a subprocess that speaks JSON-RPC 2.0 over stdin/stdout.

## Lifecycle

Each run follows the same high-level sequence:

1. Productize discovers extensions from bundled, user, and workspace scopes.
...
4. Productize sends `initialize` as the first request.
5. The extension answers with protocol version `1`, accepted capabilities, supported hooks, and lifecycle support flags.
6. Productize dispatches hooks and events during the run.
7. Productize sends `shutdown` before tearing the run scope down.

There is no restart or resume semantics in v1. Every Productize run gets a fresh subprocess.
...
- Productize -> extension: `initialize`, `execute_hook`, `on_event`, `health_check`, `shutdown`
- extension -> Productize: `host.events.*`, `host.tasks.*`, `host.runs.*`, `host.artifacts.*`, `host.prompts.render`, `host.memory.*`
...
The Host API exists so extensions do not need to shell out to `productize` or write internal files directly.
...
- `host.artifacts.*` is scoped to the workspace root and `.productize/`
