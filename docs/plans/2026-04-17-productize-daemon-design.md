...
Today Productize runs primarily as per-command execution. That keeps the system simple, but it also creates structural limits:
...
The goal of this redesign is to move Productize to a single-binary daemon model, similar in operational posture to AGH, while preserving Productize's current workflow model and keeping Markdown artifacts as first-class human-owned documents.

## Goals

- Turn Productize into a single global daemon per user/machine.
...
- Preserve compatibility with workspace-local `.productize/runs/` runtime files.
- Push JSON-RPC across every daemon boundary.
- Preserve `productize start` for compatibility.

## Approaches Considered

### 1. Thin daemon wrapper over current filesystem runtime

This approach would keep most of the current `.productize/runs/` filesystem model intact and add a small daemon shell around it.
...
- too much of AGH's domain would leak into Productize
- pushes concepts Productize does not want in v1
- risks replacing Productize's existing run engine instead of wrapping it

### 3. Recommended: Productize local platform with AGH operational patterns

This is the approved direction. Productize keeps its own workflow model and current run engine, but gains a global daemon, central operational DBs, and structured transport surfaces modeled after AGH.
...
1. `daemon/host`
   ...
   - transport startup
   - global config loading from `~/.productize`
...
3. `run service`
   ...
   - integration with the current Productize run engine
...
### 3. Runtime reuse from current Productize

The daemon does not replace Productize's current execution kernel. It wraps and reuses it.
...
- `pkg/productize/runs` reader/watch semantics where still useful
...
| AGH reference                                                        | Pattern to reuse                                       | Productize target                                        |
| -------------------------------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------- |
| `internal/config/home.go`                                            | Home path resolution and layout creation               | `~/.productize` path resolver and bootstrap layout       |
| `internal/daemon/boot.go`                                            | Locking, stale daemon cleanup, readiness boot flow     | `ensureDaemon()` and daemon auto-start                |
| `internal/daemon/daemon.go`                                          | Boot/shutdown composition root                         | Productize daemon host lifecycle                         |
| `internal/api/core/interfaces.go`                                    | Transport-facing service interfaces                    | Productize daemon service contracts                      |
...
| `internal/session/manager.go`                                        | Long-lived runtime manager patterns                    | Run/session manager around Productize's execution engine |

Automation-specific AGH surfaces are intentionally out of scope for Productize v1.

### 4. Storage model

#### 4.1 Source of truth split

Productize uses a dual ownership model:
...
All operational state lives under `~/.productize`, while Markdown workflow artifacts stay in each workspace.
...
Productize does not move to JSON-RPC everywhere.
...
- `productize sync` remains explicit
...
This keeps Productize extensible without importing AGH's automation model.
...
- `productize sync`
- `productize archive`
- `productize tasks list|show|validate|run`
- `productize reviews fetch|list|show|fix`
- `productize runs list|show|watch|attach|cancel`
- `productize daemon start|stop|status`
- `productize exec`
...
- SSE event contract
- operator messaging and cleanup behavior for the hard cutover from existing `.productize/runs/`
...
Proceed with the "Productize local platform with AGH operational patterns" approach.
...
- preservation of Productize's artifact-centric workflow model
