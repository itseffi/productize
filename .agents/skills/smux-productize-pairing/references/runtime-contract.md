...
- **Orchestrator**
  - Own the tmux session, pane lifecycle, stage routing, checkpoint resolution, and the final `productize start`.
  - Inspect `.productize/tasks/<feature-name>/_prd.md` before launch and decide whether to reuse it or route Codex through `create-prd`.
  ...
  - Re-run `productize validate-tasks --name <feature-name>` before execution.
- **Codex**
  - Own the final writes under `.productize/tasks/<feature-name>/`.
  - If the orchestrator selects the PRD path, run `/create-prd <feature-name>` first.
  - Then run `/create-techspec <feature-name>`, followed by `/create-tasks <feature-name>`.
...
- **Claude Code**
  ...
  - Avoid writing the final Productize artifacts unless the user explicitly reassigns ownership.
...
- Never use headless shortcuts in this workflow:
  ...
  - `claude -p`
  - `claude --print`
- Launch the final execution through the generated start command or `scripts/run-productize-start.sh`, not by hand.
  - The wrapper strips inherited interactive session variables before `productize start`.
...
1. **PRD gate**
   - The orchestrator checks whether `.productize/tasks/<feature-name>/_prd.md` already exists.
   - If it does, the workflow uses that PRD.
   - If it does not, Codex runs `/create-prd <feature-name>` unless the original user request explicitly said to skip PRD creation.
   - The orchestrator resolves ordinary PRD checkpoints internally and only escalates on a true blocker.
   - The phase ends when `.productize/tasks/<feature-name>/_prd.md` exists, or when the original user request explicitly waives PRD creation.
2. **TechSpec gate**
   - The PRD gate must already be resolved.
   - Codex runs `/create-techspec <feature-name>`.
   - The orchestrator resolves ordinary TechSpec checkpoints internally and only escalates on a true blocker.
   - The phase ends only when `.productize/tasks/<feature-name>/_techspec.md` exists.
3. **Task gate**
   - Codex runs `/create-tasks <feature-name>`.
   - The orchestrator resolves ordinary task-breakdown checkpoints internally and only escalates on a true blocker.
   - The phase ends only when `_tasks.md` and `task_*.md` files exist and `productize validate-tasks --name <feature-name>` exits `0`.
4. **Execution gate**
   - The orchestrator runs the generated start wrapper for `productize start`.
   - Effective command shape:
     - `env -u CODEX_THREAD_ID -u TMUX -u TMUX_PANE productize start --name <feature-name> --ide codex --model gpt-5.5 --reasoning-effort xhigh`
