---
name: smux-productize-pairing
description: Orchestrates an interactive tmux-based pairing workflow where the current agent acts as an autonomous orchestrator, Codex authors the Productize PRD/TechSpec/tasks, Claude Code challenges assumptions over tmux-bridge, and the orchestrator advances the run through productize start without a human approval loop. Use when a feature needs collaborative TUI-driven spec and task generation. Don't use for headless automation, single-agent drafting, or flows that call codex exec or claude -p.
---

# Smux Productize Pairing

## Procedures

**Step 1: Validate inputs and build the launch plan**
1. Require a feature name that resolves to `.productize/tasks/<feature-name>/`.
2. Execute `eval "$(python3 .agents/skills/smux-productize-pairing/scripts/render-session-plan.py --feature-name "<feature-name>" --repo-root "$PWD")"` to load the emitted shell assignments into the current shell.
3. Confirm that `tmux`, `tmux-bridge`, `codex`, `claude`, and `productize` are available in `PATH`.
...
6. Prefer the shell wrappers under `scripts/` when you need a concrete command string:
   ...
   - `scripts/run-productize-start.sh --feature-name "$FEATURE_NAME" --repo-root "$REPO_ROOT"`
...
6. When `create-prd` reaches a required checkpoint, have Codex message `"$ORCHESTRATOR_LABEL"` with the exact question plus its current recommendation.
...
5. When `create-techspec` reaches a required checkpoint, have Codex message `"$ORCHESTRATOR_LABEL"` with the exact question plus its current recommendation.
...
2. Apply the same orchestrator-owned checkpoint loop for the task-breakdown review required by `create-tasks`.
3. Re-run `"$VALIDATE_COMMAND"` after Codex reports success, even if `create-tasks` already validated the task set internally.
...
3. Treat the generated start wrapper as authoritative because it strips interactive Codex/tmux session variables such as `CODEX_THREAD_ID`, `TMUX`, and `TMUX_PANE` before launching `productize start`.
4. Let the run finish through the normal Productize cockpit or the output mode already encoded in the command.
5. Report the resulting `productize start` state back to the human user.
