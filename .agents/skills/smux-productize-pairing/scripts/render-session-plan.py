#!/usr/bin/env python3
"""Emit shell-safe launch variables for the smux-productize-pairing skill."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Emit shell-safe variables for the smux-productize-pairing skill."
    )
    parser.add_argument("--feature-name", required=True, help="Workflow slug under .productize/tasks/.")
    ...
    skill_root = ".agents/skills/smux-productize-pairing"
    ...
    tasks_dir = f".productize/tasks/{feature_name}"
    ...
    validate_command = shlex.join(["productize", "validate-tasks", "--name", feature_name])
    ...
    values = {
        ...
        "PRD_COMMAND": f"/create-prd {feature_name}",
        ...
        "TECHSPEC_COMMAND": f"/create-techspec {feature_name}",
        "TASKS_COMMAND": f"/create-tasks {feature_name}",
