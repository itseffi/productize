...
1. Ground in repository and PRD context.
   ...
   - If the caller provides workflow memory paths, use the installed `workflow-memory` skill before editing code.
...
4. Validate and self-review.
   ...
   - Use the installed `final-verify` skill. This step is mandatory regardless of auto-commit mode — always verify before claiming completion.
