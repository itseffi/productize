...
Substitute placeholders before invoking `productize exec`. Output is structured JSON: `blockers[]`, `risks[]`, `nits[]`, `verdict`, `summary`.
...
- Hard cuts only: any rename touches code, storage, APIs, CLI, extensions, specs, RFCs, and
  .productize/tasks/* artifacts in the same change.
