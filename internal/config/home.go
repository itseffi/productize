...
func ensureDir(path string, perm os.FileMode) error {
	...
	if err := os.MkdirAll(cleanPath, perm); err != nil {
		return fmt.Errorf("create productize directory %q: %w", cleanPath, err)
	}

	info, err := os.Stat(cleanPath)
	if err != nil {
		return fmt.Errorf("stat productize directory %q: %w", cleanPath, err)
	}
	if !info.IsDir() {
		return fmt.Errorf("config: path %q is not a directory", cleanPath)
	}
	if err := os.Chmod(cleanPath, perm); err != nil {
		return fmt.Errorf("chmod productize directory %q: %w", cleanPath, err)
	}
