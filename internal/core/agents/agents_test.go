...
import (
	...
	runtimeagent "github.com/itseffi/productize/internal/core/agent"
...
func TestDiscoverRejectsReservedAndInvalidAgentNames(t *testing.T) {
	...
	homeDir := t.TempDir()
	workspaceRoot := t.TempDir()
	writeWorkspaceAgent(t, workspaceRoot, "productize", validAgentMarkdown("Reserved", "Reserved prompt"), "")
	...
	if !errors.Is(problems["productize"], ErrReservedAgentName) {
		t.Fatalf("expected wrapped reserved-name problem to unwrap, got %v", problems["productize"])
	}
	if !errors.Is(problems["productize"].Err, ErrReservedAgentName) {
		t.Fatalf("expected reserved agent name error, got %v", problems["productize"].Err)
	}
