...
func TestFetchReviewsFiltersResolvedThreadsAndNonBotComments(t *testing.T) {
	...
	run := func(_ context.Context, args ...string) ([]byte, error) {
		...
		case len(args) >= 4 && args[0] == "repo" && args[1] == "view":
			return []byte(`{"owner":{"login":"acme"},"name":"productize"}`), nil
		case len(args) >= 2 && args[0] == "api" && strings.HasPrefix(args[1], "repos/acme/productize/pulls/259/comments"):
			return []byte(`[
...
func TestWatchStatusRejectsMalformedPayloads(t *testing.T) {
	...
	t.Run("Should reject incomplete pull request metadata", func(t *testing.T) {
		...
		run := func(_ context.Context, args ...string) ([]byte, error) {
			...
			case len(args) >= 4 && args[0] == "repo" && args[1] == "view":
				return []byte(`{"owner":{"login":"acme"},"name":"productize"}`), nil
			case len(args) >= 2 && args[0] == "api" && args[1] == "repos/acme/productize/pulls/259":
...
func testFullReviewRunner(
	...
	return func(_ context.Context, args ...string) ([]byte, error) {
		...
		case len(args) >= 2 && args[0] == "api" &&
			strings.HasPrefix(args[1], "repos/acme/productize/commits/"):
			return mustMarshalJSON(t, statuses), nil
		case len(args) >= 2 && args[0] == "api" && strings.HasPrefix(args[1], "repos/acme/productize/pulls/259/reviews"):
