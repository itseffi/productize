...
func TestDiscoverFindsNearestWorkspaceRoot(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".productize"), 0o755); err != nil {
		t.Fatalf("mkdir .productize: %v", err)
	}
...
func TestDiscoverIgnoresGlobalHomeProductizeMarker(t *testing.T) {
	homeDir := t.TempDir()
	stubWorkspaceUserHomeDir(t, func() (string, error) {
		return homeDir, nil
	})
	if err := os.MkdirAll(filepath.Join(homeDir, ".productize"), 0o755); err != nil {
		t.Fatalf("mkdir global .productize: %v", err)
	}
...
func TestSameWorkspaceMarkerDirTreatsSymlinkAndTargetAsEqual(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(realHome, ".productize"), 0o755); err != nil {
		t.Fatalf("mkdir real .productize: %v", err)
	}
	...
	if !sameWorkspaceMarkerDir(filepath.Join(realHome, ".productize"), filepath.Join(linkedHome, ".productize")) {
...
func TestLoadConfigReturnsZeroConfigWhenFileIsMissing(t *testing.T) {
	...
	if path != filepath.Join(root, ".productize", "config.toml") {
...
`)
	...
	if path != filepath.Join(homeDir, ".productize", "config.toml") {
...
func writeWorkspaceConfig(t *testing.T, workspaceRoot, content string) {
	...
	configDir := filepath.Join(workspaceRoot, ".productize")
...
func writeGlobalConfig(t *testing.T, homeDir, content string) {
	...
	configDir := filepath.Join(homeDir, ".productize")
	if err := os.MkdirAll(configDir, 0o755); err != nil {
