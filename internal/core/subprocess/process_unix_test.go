...
func TestShutdownEscalatesFromSIGTERMToSIGKILL(t *testing.T) {
	...
	readyPath := filepath.Join(tempDir, "ready.marker")

	process, err := Launch(context.Background(), LaunchConfig{
		Command: []string{os.Args[0], "-test.run=TestSubprocessSignalHelper"},
		Env: append(
			os.Environ(),
			"PRODUCTIZE_SUBPROCESS_SIGNAL_HELPER=1",
			"PRODUCTIZE_SUBPROCESS_SIGNAL_MARKER="+markerPath,
			"PRODUCTIZE_SUBPROCESS_SIGNAL_READY="+readyPath,
		),
		WaitErrorPrefix:   "wait for test subprocess",
	...
	waitForFile(t, readyPath, "signal helper readiness")
	...
	waitForFile(t, markerPath, "SIGTERM marker")
}

func TestSubprocessSignalHelper(_ *testing.T) {
	if os.Getenv("PRODUCTIZE_SUBPROCESS_SIGNAL_HELPER") != "1" {
		return
	}

	markerPath := os.Getenv("PRODUCTIZE_SUBPROCESS_SIGNAL_MARKER")
	if markerPath == "" {
		os.Exit(2)
	}
	...
	if readyPath := os.Getenv("PRODUCTIZE_SUBPROCESS_SIGNAL_READY"); readyPath != "" {
	...
	for {
		<-signals
		if err := os.WriteFile(markerPath, []byte("term"), 0o600); err != nil {
			os.Exit(3)
		}
	}
}
...
func TestProcessHelpers(t *testing.T) {
	...
	env := MergeEnvironment(
		map[string]string{"PRODUCTIZE_TEST_BASE": "1"},
		map[string]string{"PRODUCTIZE_TEST_EXTRA": "2"},
	)
	if !containsPrefix(env, "PRODUCTIZE_TEST_BASE=1") {
		t.Fatalf("missing merged base env assignment: %#v", env)
	}
	if !containsPrefix(env, "PRODUCTIZE_TEST_EXTRA=2") {
