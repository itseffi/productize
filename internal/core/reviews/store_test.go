...
func TestWriteRoundAndReadBackEntries(t *testing.T) {
	...
	reviewDir := filepath.Join(t.TempDir(), ".productize", "tasks", "demo", "reviews-001")
...
func TestDiscoverRoundsAndNextRound(t *testing.T) {
	...
	prdDir := filepath.Join(t.TempDir(), ".productize", "tasks", "demo")
...
func TestResolveUnresolvedIssuesResolvesAllLocalStatuses(t *testing.T) {
	...
	tasksDir := filepath.Join(t.TempDir(), ".productize", "tasks", "demo")
