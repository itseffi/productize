...
func TestPathHelpers(t *testing.T) {
	...
	t.Run("Should return the tasks base directory", func(t *testing.T) {
		...
		if got := model.TasksBaseDir(); got != filepath.Join(".productize", "tasks") {
	...
	t.Run("Should build workspace-aware paths", func(t *testing.T) {
		...
		if got := model.ProductizeDir(workspaceRoot); got != filepath.Join(workspaceRoot, ".productize") {
			t.Fatalf("unexpected productize dir: %q", got)
		}
	...
	t.Run("Should sanitize unsafe run identifiers", func(t *testing.T) {
		...
		if got, want := runArtifacts.RunDir, filepath.Join(".productize", "runs", "review-demo-nested"); got != want {
	...
	t.Run("Should reject dot-segment run identifiers", func(t *testing.T) {
		...
		for _, tc := range cases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if got, want := runArtifacts.RunDir, filepath.Join(".productize", "runs", "run"); got != want {
...
func TestResolveHomeRunArtifactsUsesHomeScopedRunDir(t *testing.T) {
	...
	wantRunDir := filepath.Join(homeDir, ".productize", "runs", "daemon-run-123")
