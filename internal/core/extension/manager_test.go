...
import (
	...
	runtimeevents "github.com/itseffi/productize/pkg/productize/events"
...
func TestManagerStartInitializesExtensionsPublishesReadyAndDispatchesHooks(t *testing.T) {
	...
	harness := newManagerHarness(t, managerHarnessSpec{
		...
		Env: map[string]string{
			"PRODUCTIZE_MOCK_SUPPORTED_HOOKS": "prompt.post_build",
			"PRODUCTIZE_MOCK_PATCH_JSON":      `{"message":"patched"}`,
		},
	...
	if readyPayload.ProtocolVersion != productizeProtocolVersion() {
		t.Fatalf("ready protocol_version = %q, want %q", readyPayload.ProtocolVersion, productizeProtocolVersion())
	}
...
func TestManagerStartInjectsDaemonHostCapabilityTokenIntoExtensionEnvironment(t *testing.T) {
	...
	harness := newManagerHarness(t, managerHarnessSpec{
		...
		Capabilities: []Capability{CapabilityPromptMutate},
		Hooks:        []HookDeclaration{{Event: HookPromptPostBuild}},
		Env:          map[string]string{"PRODUCTIZE_MOCK_SUPPORTED_HOOKS": "prompt.post_build"},
...
func TestHealthTimeoutMarksExtensionUnhealthyAfterTwoFailures(t *testing.T) {
	...
	harness := newManagerHarness(t, managerHarnessSpec{
		...
		Env: map[string]string{
			"PRODUCTIZE_MOCK_HEALTH_DELAY_MS": "120",
		},
	})
	defer harness.Close(t)
...
func TestOnEventForwardingDropsWhenQueueIsFull(t *testing.T) {
	...
	harness := newManagerHarness(t, managerHarnessSpec{
		...
		Capabilities: []Capability{CapabilityEventsRead},
		Env: map[string]string{
			"PRODUCTIZE_MOCK_ON_EVENT_DELAY_MS": "80",
		},
	})
	defer harness.Close(t)
...
func newManagerHarness(t *testing.T, spec managerHarnessSpec) *managerHarness {
	...
	env := map[string]string{
		"PRODUCTIZE_MOCK_MODE":        spec.Mode,
		"PRODUCTIZE_MOCK_RECORD_PATH": recordPath,
		"PRODUCTIZE_SDK_RECORD_PATH":  recordPath,
	}
	for key, value := range spec.Env {
		env[key] = value
	}
	if spec.Workflow != "" {
		env["PRODUCTIZE_MOCK_WORKFLOW"] = spec.Workflow
		env["PRODUCTIZE_SDK_WORKFLOW"] = spec.Workflow
	}
...
func buildMockExtensionBinary(t *testing.T) string {
	...
	mockExtensionBuildOnce.Do(func() {
		dir, err := os.MkdirTemp("", "productize-mock-extension-*")
		if err != nil {
