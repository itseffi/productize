...
import (
	...
	"github.com/itseffi/productize/internal/core/subprocess"
)
...
func main() {
	...
	if err := run(transport, recorder{path: os.Getenv("PRODUCTIZE_MOCK_RECORD_PATH")}); err != nil {
...
func run(transport *subprocess.Transport, rec recorder) error {
	...
	mode := strings.TrimSpace(os.Getenv("PRODUCTIZE_MOCK_MODE"))
	...
	rec.write("initialize_env", map[string]any{
		"protocol_version":      os.Getenv("PRODUCTIZE_PROTOCOL_VERSION"),
		"run_id":                os.Getenv("PRODUCTIZE_RUN_ID"),
		"parent_run_id":         os.Getenv("PRODUCTIZE_PARENT_RUN_ID"),
		"workspace_root":        os.Getenv("PRODUCTIZE_WORKSPACE_ROOT"),
		"extension_name":        os.Getenv("PRODUCTIZE_EXTENSION_NAME"),
		"extension_source":      os.Getenv("PRODUCTIZE_EXTENSION_SOURCE"),
		"host_capability_token": os.Getenv("PRODUCTIZE_HOST_CAPABILITY_TOKEN"),
	})
	...
	for {
		...
		case "on_event":
			...
			time.Sleep(durationFromEnv("PRODUCTIZE_MOCK_ON_EVENT_DELAY_MS", 0))
			if err := transport.WriteMessage(subprocess.Message{
				ID:     message.ID,
		...
		case "health_check":
			...
			case "health_timeout":
				time.Sleep(durationFromEnv("PRODUCTIZE_MOCK_HEALTH_DELAY_MS", 250*time.Millisecond))
				if err := transport.WriteMessage(subprocess.Message{
					ID:     message.ID,
		...
		case "shutdown":
			...
			time.Sleep(durationFromEnv("PRODUCTIZE_MOCK_SHUTDOWN_DELAY_MS", 0))
			if err := transport.WriteMessage(subprocess.Message{
				ID:     message.ID,
...
func buildInitializeResponse(mode string, request initializeRequest) initializeResponse {
	...
	response := initializeResponse{
		...
		SupportedHookEvents:  splitCSV(os.Getenv("PRODUCTIZE_MOCK_SUPPORTED_HOOKS")),
		Supports: initializeFlags{
			HealthCheck: mode == "health_false" || mode == "health_timeout" || os.Getenv("PRODUCTIZE_MOCK_SUPPORTS_HEALTH") == "1",
...
func callHostTasksList(transport *subprocess.Transport, rec recorder) error {
	workflow := strings.TrimSpace(os.Getenv("PRODUCTIZE_MOCK_WORKFLOW"))
...
func patchPayload(request executeHookRequest) json.RawMessage {
	...
	raw := strings.TrimSpace(os.Getenv("PRODUCTIZE_MOCK_PATCH_JSON"))
	if raw == "" {
...
func appendPatchPayload(request executeHookRequest) json.RawMessage {
	raw := strings.TrimSpace(os.Getenv("PRODUCTIZE_MOCK_APPEND_SUFFIXES_JSON"))
	if raw == "" {
		return nil
	}
