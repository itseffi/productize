...
func main() {
	rec := &recorder{path: strings.TrimSpace(os.Getenv("PRODUCTIZE_SDK_RECORD_PATH"))}
	mode := strings.TrimSpace(os.Getenv("PRODUCTIZE_SDK_REVIEW_MODE"))
	providerName := strings.TrimSpace(os.Getenv("PRODUCTIZE_SDK_REVIEW_PROVIDER"))
	if providerName == "" {
