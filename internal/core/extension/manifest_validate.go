...
func ValidateManifest(ctx context.Context, manifest *Manifest) error {
	...
	if err := validateMinProductizeVersion(manifest.Extension.MinProductizeVersion); err != nil {
		return err
	}
...
func validateExtensionInfo(info ExtensionInfo) error {
	...
	if strings.TrimSpace(info.MinProductizeVersion) == "" {
		return newManifestFieldError("extension.min_productize_version", "", "value is required")
	}
	if _, err := parseSemanticVersion(info.MinProductizeVersion); err != nil {
		return newManifestFieldError(
			"extension.min_productize_version",
			info.MinProductizeVersion,
...
func validateMinProductizeVersion(raw string) error {
	...
	if err != nil {
		return fmt.Errorf("parse current productize version %q: %w", version.Version, err)
	}
	if current.LessThan(required) {
		return newManifestFieldError(
			"extension.min_productize_version",
			raw,
			fmt.Sprintf("requires Productize %s or newer (current %s)", required, current),
		)
	}
