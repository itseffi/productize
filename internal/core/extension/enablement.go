...
const (
	userEnablementStateFileName      = ".productize-state.json"
...
func (s *EnablementStore) userStatePath(name string) string {
	return filepath.Join(s.homeDir, ".productize", "extensions", name, userEnablementStateFileName)
}

func (s *EnablementStore) workspaceStatePath() string {
	return filepath.Join(s.homeDir, ".productize", "state", workspaceEnablementStateFileName)
}
