...
func TestEnablementStorePersistsRoundTrip(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			statePath: filepath.Join(homeDir, ".productize", "extensions", "user-ext", userEnablementStateFileName),
		},
		{
			name: "workspace",
			ref: Ref{
			...
			statePath: filepath.Join(homeDir, ".productize", "state", workspaceEnablementStateFileName),
		},
	}
...
func TestEnablementStoreLoadsWorkspaceStateAcrossCanonicalRootAliases(t *testing.T) {
	...
	homeDir := t.TempDir()
	legacyRoot := filepath.Clean("/Users/pedronauck/dev/productize/agh2")
	canonicalRoot := filepath.Clean("/Users/pedronauck/Dev/productize/agh2")
...
func TestCanonicalizeExistingPathCaseWithUsesOnDiskNames(t *testing.T) {
	...
	productizeDir := filepath.Join(devDir, "productize")
	want := filepath.Join(productizeDir, "agh2")
	input := filepath.Join(homeDir, "dev", "productize", "agh2")
	...
	dirs := map[string][]os.DirEntry{
		...
		devDir:        {extensionFakeDirEntry{name: "productize"}},
		productizeDir: {extensionFakeDirEntry{name: "agh2"}},
	}
...
func TestEnablementStoreRejectsCorruptState(t *testing.T) {
	...
	writeCorruptStateFile(
		t,
		filepath.Join(homeDir, ".productize", "extensions", "user-ext", userEnablementStateFileName),
	)
	...
	writeCorruptStateFile(t, filepath.Join(homeDir, ".productize", "state", workspaceEnablementStateFileName))
...
func writeWorkspaceEnablementState(t *testing.T, homeDir string, record workspaceEnablementRecord) {
	...
	path := filepath.Join(homeDir, ".productize", "state", workspaceEnablementStateFileName)
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		t.Fatalf("MkdirAll(%q): %v", filepath.Dir(path), err)
	}
