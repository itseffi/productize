...
import (
	...
	coreprompt "github.com/itseffi/productize/internal/core/prompt"
...
func NewDefaultKernelOps(cfg DefaultKernelOpsConfig) (KernelOps, error) {
	...
	if parentRunID == "" {
		parentRunID = strings.TrimSpace(os.Getenv("PRODUCTIZE_PARENT_RUN_ID"))
	}
...
func (o *defaultKernelOps) pathOutOfScopeError(method string, rawPath string) error {
	...
	return NewPathOutOfScopeError(method, rawPath, []string{".productize/", workspaceRoot})
}
