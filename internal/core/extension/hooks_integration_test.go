...
import (
	...
	"github.com/itseffi/productize/internal/core/plan"
	"github.com/itseffi/productize/internal/core/provider"
	"github.com/itseffi/productize/internal/core/reviews"
	runcore "github.com/itseffi/productize/internal/core/run"
)
...
func TestHookDispatchIntegrationAcrossArtifactWritePhases(t *testing.T) {
	...
	if got := preWritePayload["path"]; got != ".productize/artifacts/note.txt" {
		t.Fatalf("artifact.pre_write path = %#v, want %q", got, ".productize/artifacts/note.txt")
	}
...
func discoveredRecordedExtension(
	...
	env := map[string]string{
		"PRODUCTIZE_MOCK_MODE":            "normal",
		"PRODUCTIZE_MOCK_RECORD_PATH":     recordPath,
		"PRODUCTIZE_MOCK_SUPPORTED_HOOKS": joinHookNames(hooks),
	}
	if strings.TrimSpace(patchJSON) != "" {
		env["PRODUCTIZE_MOCK_PATCH_JSON"] = patchJSON
	}
	if len(suffixes) > 0 {
		...
		env["PRODUCTIZE_MOCK_APPEND_SUFFIXES_JSON"] = string(suffixesJSON)
	}
	...
	return DiscoveredExtension{
		...
		ExtensionDir: root,
		ManifestPath: filepath.Join(root, "productize.toml"),
		Enabled:      true,
	}
}
