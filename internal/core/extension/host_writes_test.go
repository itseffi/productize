...
import (
	...
	"github.com/itseffi/productize/internal/core/kernel/commands"
	"github.com/itseffi/productize/internal/core/memory"
...
func TestHostTasksCreateReturnsSequentialNumberAndEmitsTaskFileUpdated(t *testing.T) {
	...
	if task.Path != ".productize/tasks/extensibility/task_02.md" {
		t.Fatalf("task.Path = %q, want .productize/tasks/extensibility/task_02.md", task.Path)
	}
...
func TestHostTasksCreateNormalizesFrontmatterAndTaskBody(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join(rt.root, ".productize", "tasks", "extensibility", "task_01.md"))
	if err != nil {
...
func TestHostTasksCreateUpdateIndexAppendsTaskListRow(t *testing.T) {
	...
	tasksDir := filepath.Join(rt.root, ".productize", "tasks", "extensibility")
...
func TestHostMemoryWriteWithoutTaskFileUsesWorkflowMemory(t *testing.T) {
	...
	if response.Path != ".productize/tasks/extensibility/memory/MEMORY.md" {
		t.Fatalf("response.Path = %q, want %q", response.Path, ".productize/tasks/extensibility/memory/MEMORY.md")
	}

	content, err := os.ReadFile(filepath.Join(rt.root, ".productize", "tasks", "extensibility", "memory", "MEMORY.md"))
	if err != nil {
...
func TestHostArtifactsWriteWritesScopedFileAndEmitsArtifactUpdated(t *testing.T) {
	...
	result, err := rt.router.Handle(
		...
		mustJSON(t, ArtifactWriteRequest{
			Path:    ".productize/artifacts/note.txt",
	...
	if response.Path != ".productize/artifacts/note.txt" {
		t.Fatalf("response.Path = %q, want %q", response.Path, ".productize/artifacts/note.txt")
	}
	...
	content, err := os.ReadFile(filepath.Join(rt.root, ".productize", "artifacts", "note.txt"))
	if err != nil {
	...
	if payload.Path != ".productize/artifacts/note.txt" {
		t.Fatalf("payload.Path = %q, want %q", payload.Path, ".productize/artifacts/note.txt")
	}
}
...
func TestHostArtifactsWriteCancelByExtensionPreventsWrite(t *testing.T) {
	...
	if _, statErr := os.Stat(
		filepath.Join(rt.root, ".productize", "artifacts", "note.txt"),
	); !errors.Is(
		statErr,
		os.ErrNotExist,
	) {
