	...
	result, err := rt.router.Handle(context.Background(), "ext", "host.prompts.render", mustJSON(t, PromptRenderRequest{
		...
		Params: PromptRenderParams{
			...
			Memory: &PromptMemoryContext{
				WorkflowPath: "/repo/.productize/tasks/extensibility/memory/MEMORY.md",
				TaskPath:     "/repo/.productize/tasks/extensibility/memory/task_06.md",
			},
		},
	}))
	if err != nil {
		...
		Params: PromptRenderParams{
			...
			BatchGroups: map[string][]PromptIssueRef{
				"task_06": {
					{
						Name:    "task_06.md",
						AbsPath: filepath.Join(rt.root, ".productize", "tasks", "extensibility", "task_06.md"),
...
func TestDefaultKernelOpsTasksDirForWorkflowValidation(t *testing.T) {
	...
	if want := filepath.Join(ops.workspaceRoot, ".productize", "tasks", "extensibility"); got != want {
...
func TestDefaultKernelOpsWorkspaceRelativeAndPathWithinRoot(t *testing.T) {
	...
	nested := filepath.Join(root, ".productize", "tasks", "extensibility", "task_06.md")
	if got := ops.workspaceRelative(nested); got != ".productize/tasks/extensibility/task_06.md" {
		t.Fatalf("workspaceRelative(nested) = %q, want .productize/tasks/extensibility/task_06.md", got)
	}
