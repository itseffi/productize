...
func TestValidateManifestRejectsInvalidValues(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			current: "1.0.0",
			mutate: func(manifest *Manifest) {
				manifest.Extension.MinProductizeVersion = "2.0.0"
			},
			wantSubstr: `extension.min_productize_version="2.0.0": requires Productize 2.0.0 or newer (current 1.0.0)`,
		},
	}
