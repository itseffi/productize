...
func TestDiscoveryIncludesWorkspaceExtensionEnabledThroughCanonicalRootAlias(t *testing.T) {
	...
	writeManifestJSON(
		t,
		workspaceExtensionDir(canonicalRoot, "qa-workflow"),
		manifestFixture("qa-workflow"),
	)
	...
	if got := result.Extensions[0].Ref.Name; got != "qa-workflow" {
		t.Fatalf("Extensions[0].Ref.Name = %q, want qa-workflow", got)
	}
	if !result.Extensions[0].Enabled {
		t.Fatal("Extensions[0].Enabled = false, want true")
	}
...
func TestDiscoveryFailureErrorIncludesContext(t *testing.T) {
	failure := DiscoveryFailure{
		...
		ExtensionDir: "/tmp/workspace/.productize/extensions/broken",
		Err:          errors.New("boom"),
	}
