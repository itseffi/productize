...
func TestBuildDefaultDispatchesRunStartAndDelegatesToPrepareAndExecute(t *testing.T) {
	fake := &fakeOperations{
		prepareResult: &model.SolvePreparation{
			...
			InputDir:     "demo",
			InputDirPath: "/workspace/.productize/tasks/demo",
		},
	}
	dispatcher := BuildDefault(testKernelDeps(fake))
	...
	expectedArtifactsDir := "/workspace/.productize/runs/run-123"
...
func TestBuildDefaultDispatchesAllPhaseACommandsSequentially(t *testing.T) {
	fake := &fakeOperations{
		...
		migrateResult: &model.MigrationResult{Target: "/workspace/.productize/tasks/demo"},
		syncResult:    &model.SyncResult{Target: "/workspace/.productize/tasks/demo"},
		archiveResult: &model.ArchiveResult{Target: "/workspace/.productize/tasks/demo"},
	}
	dispatcher := BuildDefault(testKernelDeps(fake))
