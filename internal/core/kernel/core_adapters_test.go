...
func TestDispatchRunAdapterBuildsRunStartCommand(t *testing.T) {
	...
	err := dispatchRunAdapter(context.Background(), core.Config{
		...
		Name:                   "demo",
		TasksDir:               "/workspace/.productize/tasks/demo",
		Mode:                   core.ModePRReview,
		IDE:                    core.IDECodex,
		Model:                  "gpt-5.5",
		ReviewsDir:             "/workspace/.productize/tasks/demo/reviews-001",
	...
	if runtimeCfg.ReviewsDir != "/workspace/.productize/tasks/demo/reviews-001" {
			...
			run: func(t *testing.T, dispatcher *Dispatcher) {
				...
				handler := &workspaceMigrateCaptureHandler{
					result: commands.WorkspaceMigrateResult{
						Result: &model.MigrationResult{Target: "/workspace/.productize/tasks/demo"},
					},
				}
				Register(dispatcher, handler)
				...
				result, err := dispatchMigrateAdapter(context.Background(), core.MigrationConfig{
					WorkspaceRoot: "/workspace",
					RootDir:       "/workspace/.productize/tasks",
					Name:          "demo",
					TasksDir:      "/workspace/.productize/tasks/demo",
					ReviewsDir:    "/workspace/.productize/tasks/demo/reviews-001",
					DryRun:        true,
				...
				if handler.got.RootDir != "/workspace/.productize/tasks" || handler.got.ReviewsDir == "" ||
					!handler.got.DryRun {
			...
			run: func(t *testing.T, dispatcher *Dispatcher) {
				...
				handler := &workflowSyncCaptureHandler{
					result: commands.WorkflowSyncResult{
						Result: &model.SyncResult{Target: "/workspace/.productize/tasks/demo"},
					},
				}
				Register(dispatcher, handler)
				...
				if handler.got.RootDir != "/workspace/.productize/tasks" || handler.got.TasksDir == "" {
			...
			run: func(t *testing.T, dispatcher *Dispatcher) {
				...
				handler := &workflowArchiveCaptureHandler{
					result: commands.WorkflowArchiveResult{
						Result: &model.ArchiveResult{Target: "/workspace/.productize/tasks/demo"},
					},
				}
				Register(dispatcher, handler)
