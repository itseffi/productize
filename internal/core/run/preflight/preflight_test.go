...
func TestRunValidationFormCopyPromptCopiesToClipboardAfterExit(t *testing.T) {
	...
	if !strings.Contains(copied, "Fix the Productize task metadata files below.") {
	...
	if strings.Contains(stderr.String(), "Fix the Productize task metadata files below.") {
...
func TestRunValidationFormCopyPromptFallsBackToStderrWhenClipboardFails(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Fix prompt:",
		"Fix the Productize task metadata files below.",
	} {
