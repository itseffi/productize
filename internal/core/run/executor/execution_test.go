...
import (
	...
	"github.com/itseffi/productize/internal/core/run/internal/worktree"
...
func TestAfterJobSuccessResolvesNewlyResolvedIssuesAndRefreshesMeta(t *testing.T) {
	tmpDir := t.TempDir()
	reviewDir := filepath.Join(tmpDir, ".productize", "tasks", "demo", "reviews-001")
...
func TestAfterJobSuccessRefreshesTaskMetaForPRDTasks(t *testing.T) {
	tmpDir := t.TempDir()
	tasksDir := filepath.Join(tmpDir, ".productize", "tasks", "demo")
...
func TestAfterTaskJobSuccessSkipsMarkCompletedWhenWorkspaceUnchanged(t *testing.T) {
	...
	tasksDir := filepath.Join(workspace, ".productize", "tasks", "demo")
	if err := os.MkdirAll(tasksDir, 0o755); err != nil {
		t.Fatalf("mkdir tasks dir: %v", err)
	}
	writeRunTaskFile(t, tasksDir, "task_01.md", "pending")
