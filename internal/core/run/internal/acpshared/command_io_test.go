...
func TestCreateACPSessionForwardsMCPServersOnNewSession(t *testing.T) {
	...
	servers := []model.MCPServer{{
		Stdio: &model.MCPServerStdio{
			Name:    "productize",
			Command: "/tmp/productize-test",
			Args:    []string{"mcp-serve", "--server", "productize"},
		},
	}}
	...
	if client.createReq.MCPServers[0].Stdio == nil ||
		client.createReq.MCPServers[0].Stdio.Name != "productize" {
...
func TestSetupSessionExecutionEmitsReusableAgentLifecycleSetupEventsOnNewAndResume(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			execution, err := SetupSessionExecution(SessionSetupRequest{
				...
				Job: &job{
					...
					MCPServers: []model.MCPServer{
						{Stdio: &model.MCPServerStdio{Name: "productize", Command: "/tmp/productize-test"}},
			...
			if got, want := mcpMerged.MCPServers, []string{"productize", "filesystem"}; !slices.Equal(got, want) {
