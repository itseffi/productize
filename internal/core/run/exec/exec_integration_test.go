...
import (
	...
	"github.com/itseffi/productize/internal/core/run/internal/acpshared"
...
func TestExecuteExecTextModePrintsOnlyFinalAssistantResponse(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(tmpDir, ".productize", "runs")); !os.IsNotExist(err) {
...
func TestExecuteExecPersistedRunCanResumeSameSession(t *testing.T) {
	...
	installACPHelperOnPath(
		...
		[]runACPHelperScenario{{
			...
			ExpectedNewSessionMCPServerNames: []string{"productize", "filesystem"},
		}},
		[]runACPHelperScenario{{
			SessionID:                        "sess-1",
		...
		[]runACPHelperScenario{{
			...
			ExpectedLoadSessionMCPServerNames: []string{"productize", "filesystem"},
			SupportsLoadSession:               true,
			SessionMeta:                       map[string]any{"agentSessionId": "agent-1"},
