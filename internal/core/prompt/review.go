...
func buildReviewScopeSection(ctx reviewPromptContext) string {
	...
	sb.WriteString(
		"- Do not call provider-specific scripts, `gh` mutations, or other external resolution commands. Productize resolves provider threads after the batch succeeds.\n",
	)
	sb.WriteString("- Do not edit issue files outside this batch.\n")
	sb.WriteString(
		"- Use installed `final-verify` before claiming this batch is complete or creating an automatic commit.\n",
	)
	sb.WriteString("</critical>\n\n")
...
func buildReviewExecutionSection(ctx reviewPromptContext) string {
	...
	sb.WriteString(
		"6. Use `final-verify` to identify and run the repository's real verification commands before finishing or committing this batch.\n",
	)
	if ctx.AutoCommit {
		sb.WriteString(
