...
// already authorized execution by invoking `productize tasks run`; the agent must
...
func buildPRDKickoffDirective(taskName string) string {
	return fmt.Sprintf(
		"<action_required>\n"+
			"Begin work on **%s** immediately using the installed `execute-task` skill.\n"+
			...
			"brief and start executing the execute-task workflow now.\n"+
			"</action_required>",
		taskName,
	)
}
...
func buildPRDClosingDirective(taskName string) string {
	return fmt.Sprintf(
		"<begin_now>\n"+
			"You have the full brief above. Start the execute-task workflow on **%s** "+
...
- ` + "`workflow-memory`" + `: required when workflow memory paths are provided for this task
- ` + "`execute-task`" + `: required end-to-end workflow for a PRD task
- ` + "`final-verify`" + `: required before any completion claim or automatic commit
</required_skills>`
}
...
func buildPRDExecutionRulesSection(prdDir string, autoCommit bool) string {
	...
	sb.WriteString(
		"- Use installed `final-verify` before any completion claim or automatic commit.\n",
	)
	if autoCommit {
...
func buildWorkflowMemorySection(memory *WorkflowMemoryContext) string {
	...
	sb.WriteString("- Use installed `workflow-memory` before editing code and before finishing the task.\n")
	sb.WriteString(
...
func buildPRDSystemPromptAddendum(memory *WorkflowMemoryContext) string {
	...
	lines := []string{
		"<workflow_memory>",
		"You MUST use installed `workflow-memory` for this PRD task.",
...
func buildTaskFilesSection(taskAbsPath, tasksFile, prdDir string, autoCommit bool) string {
	...
	sb.WriteString("- Use these exact paths when `execute-task` updates task tracking.\n")
	sb.WriteString(
