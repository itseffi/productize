...
func TestRegisterIgnoresGlobalHomeProductizeMarker(t *testing.T) {
	homeDir := t.TempDir()
...
func TestResolveOrRegisterIgnoresGlobalHomeProductizeMarker(t *testing.T) {
	homeDir := t.TempDir()
	t.Setenv("HOME", homeDir)
...
func TestCanonicalizeExistingPathCaseWithUsesOnDiskNames(t *testing.T) {
	...
	want := filepath.Join(productizeDir, "agh")
	input := filepath.Join(homeDir, "dev", "productize", "agh")

	dirs := map[string][]os.DirEntry{
		root:          {fakeDirEntry{name: "Users"}},
		usersDir:      {fakeDirEntry{name: "pedronauck"}},
		homeDir:       {fakeDirEntry{name: "Dev"}},
		devDir:        {fakeDirEntry{name: "productize"}},
		productizeDir: {fakeDirEntry{name: "agh"}},
	}
...
func TestCanonicalizeExistingPathCaseWithFallsBackToCleanPathWhenParentsCannotBeRead(t *testing.T) {
	...
	root := testAbsoluteRoot(t)
	input := filepath.Join(root, "Users", "pedronauck", "Dev", "productize", "agh")
