...
var routeExclusions = map[string]struct{}{
...
func TestOpenAPIContractMatchesRegisteredRoutes(t *testing.T) {
	...
	if diff := diffRoutes(expected, specRoutes); diff != "" {
		t.Fatalf("OpenAPI path set drifted:\n%s", diff)
	}
	...
	registered := registeredRouteKeys(engine.Routes())
	if diff := diffRoutes(expected, registered); diff != "" {
		t.Fatalf("registered route set drifted:\n%s", diff)
	}
}

func TestOpenAPIContractKeepsWorkspaceContextAndProblemSemantics(t *testing.T) {
	t.Parallel()
	...
	for _, routeKey := range []string{
		...
		routeKey := routeKey
		t.Run("Should advertise transport security for "+routeKey, func(t *testing.T) {
			t.Parallel()

			operation := getOperation(t, spec, routeKey)
			if !hasResponse(operation, "403") {
				t.Fatalf("%s is missing 403 transport security response", routeKey)
			}
		})
	}
}
...
func loadOpenAPISpec(t *testing.T) map[string]any {
	...
	path := filepath.Join("..", "..", "..", "openapi", "productize-daemon.json")
...
func registeredRouteKeys(routes gin.RoutesInfo) []string {
	keys := make([]string, 0, len(routes))
	seen := make(map[string]struct{}, len(routes))
	for _, route := range routes {
		key := route.Method + " " + ginPathParamPattern.ReplaceAllString(route.Path, "{$1}")
		if _, skip := routeExclusions[key]; skip {
			continue
		}
		if _, ok := seen[key]; ok {
			continue
