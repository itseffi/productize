...
func detailsForError(err error) map[string]any {
	...
	if errors.As(err, &globalSchemaErr) {
		return map[string]any{
			...
			"remediation":     "upgrade this Productize binary before opening the daemon catalog",
		}
	}
	...
	if errors.As(err, &runSchemaErr) {
		return map[string]any{
			...
			"remediation":     "upgrade this Productize binary before opening the run database",
		}
	}
