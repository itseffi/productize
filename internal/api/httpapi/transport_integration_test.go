...
func TestHTTPServesAPIOnly(t *testing.T) {
	t.Parallel()
	...
	statusCode, _, _ := mustRequest(t, http.DefaultClient, http.MethodGet, baseURL+"/", nil, nil)
	if statusCode != http.StatusNotFound {
		t.Fatalf("GET / status = %d, want %d", statusCode, http.StatusNotFound)
	}
...
func TestHTTPUnknownRoutesRemainNotFound(t *testing.T) {
	t.Parallel()
	...
	if statusCode != http.StatusNotFound {
		t.Fatalf("GET deep link status = %d, want %d", statusCode, http.StatusNotFound)
	}

	statusCode, _, _ = mustRequest(t, http.DefaultClient, http.MethodGet, baseURL+"/api/missing", nil, nil)
	if statusCode != http.StatusNotFound {
...
func TestHTTPAndUDSRemainAPIOnly(t *testing.T) {
	t.Parallel()
	...
	statusCode, _, _ := mustRequest(t, http.DefaultClient, http.MethodGet, baseURL+"/", nil, nil)
	if statusCode != http.StatusNotFound {
		t.Fatalf("HTTP GET / status = %d, want %d", statusCode, http.StatusNotFound)
	}

	statusCode, _, _ = mustRequest(t, udsClient, http.MethodGet, "http://unix/", nil, nil)
	if statusCode != http.StatusNotFound {
...
func TestHTTPServerPersistsActualPortInDaemonInfo(t *testing.T) {
	...
	homeRoot := filepath.Join(t.TempDir(), ".productize")
