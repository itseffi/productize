...
func TestReadStateWriteStateRoundTrip(t *testing.T) {
	...
	want := &StateEntry{
		...
		LatestRelease: ReleaseInfo{
			Version:     "1.2.3",
			URL:         "https://github.com/itseffi/productize/releases/tag/v1.2.3",
...
func TestStateFilePathUsesXDGConfigHomeWhenSet(t *testing.T) {
	xdgHome := filepath.Join(os.TempDir(), "productize-xdg")
	...
	want := filepath.Join(xdgHome, "productize", "state.yml")
	if path != want {
...
func TestStateFilePathFallsBackToHomeConfigDir(t *testing.T) {
	...
	homeDir := filepath.Join(os.TempDir(), "productize-home")
	...
	want := filepath.Join(homeDir, ".config", "productize", "state.yml")
	if path != want {
		t.Fatalf("unexpected state file path: want %q, got %q", want, path)
	}
}
