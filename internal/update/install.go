...
const homebrewFormulaUpgradeCommand = "brew upgrade itseffi/productize/productize"
...
func Upgrade(ctx context.Context, currentVersion string, stdout io.Writer) error {
	...
	case InstallNPM:
		return printManagedUpgradeCommand(stdout, "npm install -g @productize/cli@latest")
	case InstallGo:
		return printManagedUpgradeCommand(stdout, "go install github.com/itseffi/productize/cmd/productize@latest")
	default:
		...
		if newer == nil {
			_, writeErr := fmt.Fprintln(stdout, "productize is already up to date")
			return writeErr
		}

		_, writeErr := fmt.Fprintf(stdout, "Updated productize to %s\n", newer.Version)
		return writeErr
	}
}
