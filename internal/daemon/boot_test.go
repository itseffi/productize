...
func TestStartReturnsHomeResolutionError(t *testing.T) {
	...
	restore := stubResolveDaemonHomePaths(t, func() (productizeconfig.HomePaths, error) {
		return productizeconfig.HomePaths{}, homeErr
	})
	defer restore()
...
func mustHomePaths(t *testing.T) productizeconfig.HomePaths {
	t.Helper()
	...
	homeRoot, err := os.MkdirTemp(baseDir, "productize-daemon-*")
	if err != nil {
	...
	paths, err := productizeconfig.ResolveHomePathsFrom(filepath.Join(homeRoot, ".productize"))
	if err != nil {
...
func stubResolveDaemonHomePaths(
	t *testing.T,
	fn func() (productizeconfig.HomePaths, error),
) func() {
	t.Helper()
