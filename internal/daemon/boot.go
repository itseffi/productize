...
var resolveDaemonHomePaths = productizeconfig.ResolveHomePaths
...
func Start(ctx context.Context, opts StartOptions) (_ StartResult, retErr error) {
	...
	if err := productizeconfig.EnsureHomeLayout(resolved.HomePaths); err != nil {
...
func (h *Host) Paths() productizeconfig.HomePaths {
	if h == nil {
		return productizeconfig.HomePaths{}
	}
	return h.paths
}
...
func normalizeProbeOptions(
	paths productizeconfig.HomePaths,
	opts ProbeOptions,
) (productizeconfig.HomePaths, func(int) bool, func(context.Context, Info) error, error) {
	resolvedPaths := paths
	if strings.TrimSpace(resolvedPaths.HomeDir) == "" {
		loadedPaths, err := resolveDaemonHomePaths()
		if err != nil {
			return productizeconfig.HomePaths{}, nil, nil, err
		}
