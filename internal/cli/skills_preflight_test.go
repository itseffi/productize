...
func TestRequiredSkillStateHelpers(t *testing.T) {
	...
	state := requiredSkillState{
		AgentName:         "codex",
		BundledSkillNames: []string{"execute-task", "final-verify"},
		Bundled: setup.VerifyResult{
			...
			Skills: []setup.VerifiedSkill{
				{Skill: setup.Skill{Name: "final-verify"}, State: setup.VerifyStateDrifted},
	...
	if got := strings.Join(state.MissingSkillNames(), ","); got != "execute-task,ext-missing" {
		t.Fatalf("unexpected missing skill names: %q", got)
	}
	if got := strings.Join(state.DriftedSkillNames(), ","); got != "ext-drift,final-verify" {
	...
	if got := strings.Join(state.BlockingMissingSkillNames(), ","); got != "execute-task" {
	...
	if got := strings.Join(state.RefreshSkillNames(), ","); got != "ext-drift,ext-missing,final-verify" {
...
func TestBuildMissingSkillErrorUsesScopeSpecificGuidance(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:        "project scope",
			scope:       setup.InstallScopeProject,
			wantSnippet: "Run `productize setup --agent codex` to update project skills",
		},
		{
			name:        "global scope",
			scope:       setup.InstallScopeGlobal,
			wantSnippet: "Run `productize setup --agent codex --global` to update global skills",
		},
		{
			name:        "unknown scope",
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := buildMissingSkillError("productize tasks run", "codex", state)
...
func TestEnsureBundledSkillsCurrent(t *testing.T) {
	...
	baseState := requiredSkillState{
		AgentName:         "codex",
		BundledSkillNames: []string{"execute-task"},
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: "missing skills remain: execute-task",
		},
		{
			name: "drift remains",
			bundled: setup.VerifyResult{
...
func TestRefreshBundledSkillsInstallsBundledAndExtensionSkills(t *testing.T) {
	...
	state.installBundledSkills = func(cfg setup.InstallConfig) (*setup.Result, error) {
		...
		if got := strings.Join(cfg.SkillNames, ","); got != "execute-task,final-verify" {
...
func TestVerifyRequiredSkillStateUsesSetupAgentNameAndExtensionScopeHint(t *testing.T) {
	...
	t.Run("Should use setup agent name and extension scope hint", func(t *testing.T) {
		...
		state.listBundledSkills = func() ([]setup.Skill, error) {
			return []setup.Skill{{Name: "execute-task"}, {Name: "final-verify"}}, nil
		}
