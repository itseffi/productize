...
func TestUpgradeHelpShowsNoUnexpectedFlags(t *testing.T) {
	...
	required := []string{
		"Upgrade productize using the appropriate installation flow for this machine.",
...
func TestDaemonDocsUseCurrentCommandSurface(t *testing.T) {
	...
	forbidden := []string{
		"productize start",
		"productize validate-tasks",
		"productize fetch-reviews",
		"productize fix-reviews",
		".productize/runs/<run-id>/extensions.jsonl",
	...
	if !containsAll(string(architecture), "~/.productize/runs/<run-id>/run.db", "hook_runs") {
...
func TestBuildConfigUsesTaskFlagsForStartWorkflow(t *testing.T) {
	...
	state.tasksDir = ".productize/tasks/multi-repo"
	...
	if cfg.TasksDir != ".productize/tasks/multi-repo" {
...
func TestFormInputsApplyForReviewWorkflow(t *testing.T) {
	...
	cmd.Flags().Bool("include-resolved", false, "include resolved")
		...
		name:            "my-feature",
		reviewsDir:      ".productize/tasks/my-feature/reviews-001",
		round:           "2",
		batchSize:       "3",
	...
	if state.reviewsDir != ".productize/tasks/my-feature/reviews-001" {
...
func TestFormInputsApplyPreservesPrefilledOptionalValuesWhenLeftBlank(t *testing.T) {
	...
	cmd := newTestCommand(state)
	state.round = 2
	state.reviewsDir = ".productize/tasks/my-feature/reviews-001"
	state.model = "gpt-5.5"
...
func TestFormInputsApplyForStartWorkflow(t *testing.T) {
	...
	if state.tasksDir != ".productize/tasks/multi-repo" {
...
func TestFetchReviewsWithPartialFlagsSkipsFormAndReturnsValidationError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "productize reviews fetch requires --name") {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(output, "Error: productize reviews fetch requires --name") {
...
`)
	...
	if !strings.Contains(err.Error(), "apply workspace defaults for productize tasks run") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
`)
	...
	_, err := executeRootCommand("sync", "--name", "demo", "--tasks-dir", ".productize/tasks/demo")
	if err == nil {
...
func TestRunPreparedBlocksStartWhenBundledSkillsAreMissing(t *testing.T) {
	...
	state.listBundledSkills = func() ([]setup.Skill, error) {
		return []setup.Skill{
			{Name: "execute-task", Description: "Execute a task"},
			{Name: "final-verify", Description: "Verify completion"},
		}, nil
	}
	state.verifyBundledSkills = func(setup.VerifyConfig) (setup.VerifyResult, error) {
		return setup.VerifyResult{
			...
			Skills: []setup.VerifiedSkill{
				{Skill: setup.Skill{Name: "execute-task"}, State: setup.VerifyStateMissing},
				{Skill: setup.Skill{Name: "final-verify"}, State: setup.VerifyStateMissing},
			},
		}, nil
	}
	...
	if !strings.Contains(err.Error(), "productize setup --agent codex") {
		t.Fatalf("expected setup instruction, got %v", err)
	}
	if !strings.Contains(err.Error(), "execute-task, final-verify") {
...
func TestRunPreparedBlocksFixReviewsWhenBundledSkillsAreMissing(t *testing.T) {
	...
	state.listBundledSkills = func() ([]setup.Skill, error) {
		return []setup.Skill{
			{Name: "fix-reviews", Description: "Fix reviews"},
			{Name: "final-verify", Description: "Verify completion"},
		}, nil
	}
	state.verifyBundledSkills = func(setup.VerifyConfig) (setup.VerifyResult, error) {
		return setup.VerifyResult{
			...
			Skills: []setup.VerifiedSkill{
				{Skill: setup.Skill{Name: "fix-reviews"}, State: setup.VerifyStateCurrent},
				{Skill: setup.Skill{Name: "final-verify"}, State: setup.VerifyStateMissing},
			},
		}, nil
	}
	...
	if !strings.Contains(err.Error(), "productize setup --agent claude-code") {
		t.Fatalf("expected setup instruction, got %v", err)
	}
	if !strings.Contains(err.Error(), "project-local install is missing: final-verify") {
...
func TestRunPreparedRefreshesDriftedSkillsBeforeRunningWorkflow(t *testing.T) {
	...
	state.verifyBundledSkills = func(cfg setup.VerifyConfig) (setup.VerifyResult, error) {
		verifyCalls++
		if !reflect.DeepEqual(cfg.SkillNames, []string{"execute-task", "final-verify"}) {
		...
		if verifyCalls == 1 {
			return setup.VerifyResult{
				...
				Skills: []setup.VerifiedSkill{
					{Skill: setup.Skill{Name: "execute-task"}, State: setup.VerifyStateDrifted},
					{Skill: setup.Skill{Name: "final-verify"}, State: setup.VerifyStateCurrent},
				},
			}, nil
		}
		return setup.VerifyResult{
			...
			Mode:  setup.InstallModeCopy,
			Skills: []setup.VerifiedSkill{
				{Skill: setup.Skill{Name: "execute-task"}, State: setup.VerifyStateCurrent},
				{Skill: setup.Skill{Name: "final-verify"}, State: setup.VerifyStateCurrent},
			},
		}, nil
	}
	...
	state.confirmSkillRefresh = func(_ *cobra.Command, prompt skillRefreshPrompt) (bool, error) {
		...
		if !reflect.DeepEqual(prompt.DriftedSkills, []string{"execute-task"}) {
	...
	if !strings.Contains(output.String(), "Updated required Productize skills for Codex (project scope).") {
