...
func (s *commandState) handleBundledSkillDrift(
	...
	_, _ = fmt.Fprintf(
		cmd.OutOrStdout(),
		"Updated required Productize skills for %s (%s scope).\n",
...
func buildMissingSkillError(commandPath, agentName string, result requiredSkillState) error {
	...
	case setup.InstallScopeProject:
		return fmt.Errorf(
			"%s requires Productize and enabled extension skills for %s. The project-local install is missing: %s. Run `productize setup --agent %s` to update project skills, or `productize setup --agent %s --global` to install globally",
			commandPath,
	...
	case setup.InstallScopeGlobal:
		return fmt.Errorf(
			"%s requires Productize and enabled extension skills for %s. The global install is missing: %s. Run `productize setup --agent %s --global` to update global skills, or `productize setup --agent %s` to install project-local skills",
			commandPath,
			result.AgentDisplayName(),
			missing,
			agentName,
	...
	default:
		return fmt.Errorf(
			"%s requires Productize and enabled extension skills for %s. No compatible skills were found in project or global scope; missing skills: %s. Run `productize setup --agent %s` to install project-local skills, or `productize setup --agent %s --global` to install globally",
			commandPath,
			result.AgentDisplayName(),
			missing,
			agentName,
...
func printBundledSkillDriftWarning(cmd *cobra.Command, result requiredSkillState, suffix string) {
	_, _ = fmt.Fprintf(
		cmd.OutOrStdout(),
		"Warning: required Productize skills for %s differ from the installed %s scope: %s; %s.\n",
		result.AgentDisplayName(),
...
func confirmSkillRefreshPrompt(cmd *cobra.Command, prompt skillRefreshPrompt) (bool, error) {
	_, _ = fmt.Fprintf(
		cmd.OutOrStdout(),
		"Required Productize skills for %s differ from the installed %s scope: %s.\n",
