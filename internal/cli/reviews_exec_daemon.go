...
func newReviewsFetchCommandWithDefaults(defaults commandStateDefaults) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Fetch review comments from a provider and write them into .productize/tasks/<name>/reviews-NNN/.\n\n" +
			"When --provider is omitted, Productize can load its default from " +
			"~/.productize/config.toml or .productize/config.toml.",
		Example: `  productize reviews fetch my-feature --provider coderabbit --pr 259
  ...
  productize reviews fetch --name my-feature`,
	...
	cmd.Flags().StringVar(&state.name, "name", "", "Workflow name (used for .productize/tasks/<name>)")
...
func newReviewsFixCommandWithDefaults(defaults commandStateDefaults) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceUsage: true,
		Args:         cobra.MaximumNArgs(1),
		Long: `Process review issue markdown files from .productize/tasks/<name>/reviews-NNN/ and run the configured AI agent
to remediate review feedback.

Most runtime defaults can be supplied by ~/.productize/config.toml and overridden by
.productize/config.toml. In interactive terminals the command
opens the run cockpit by default; in non-TTY environments it falls back to headless streaming.`,
		Example: `  productize reviews fix my-feature --ide codex --concurrent 2 --batch-size 3
  productize reviews fix --name my-feature --round 2
  productize reviews fix my-feature --format json --round 2
  ...
  productize reviews fix --name my-feature`,
	...
	cmd.Flags().StringVar(
		...
		"reviews-dir",
		"",
		"Path to a review round directory (.productize/tasks/<name>/reviews-NNN)",
	)
...
through the textual run stream.`,
  ...
  productize reviews watch tools-registry --provider coderabbit --pr 85 --format raw-json`,
...
func writeStartedReviewWatchRun(cmd *cobra.Command, run apicore.Run) error {
	...
	if run.PresentationMode == attachModeDetach {
		message = fmt.Sprintf(
			"review watch started: %s (running in background; follow with: productize runs watch %s)\n",
			run.RunID,
			run.RunID,
		)
	}
...
func reviewRoundDirForWorkflow(workspaceRoot string, workflowSlug string, round int) string {
	return filepathJoin(workspaceRoot, ".productize", "tasks", workflowSlug, fmt.Sprintf("reviews-%03d", round))
}
