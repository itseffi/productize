...
Validation failures return exit code 1. Filesystem, config, or flag errors return exit code 2.`,
		Example: `  productize tasks validate --name my-feature
  productize tasks validate --tasks-dir .productize/tasks/my-feature
  productize tasks validate --tasks-dir .productize/tasks/my-feature --format json`,
		RunE: state.run,
	}

	cmd.Flags().StringVar(&state.name, "name", "", "Task workflow name (used for .productize/tasks/<name>)")
	cmd.Flags().StringVar(&state.tasksDir, "tasks-dir", "", "Path to tasks directory (.productize/tasks/<name>)")
	cmd.Flags().
