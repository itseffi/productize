...
func TestPublishExtensionSDKsTargetRequiresVerificationAndPublicAccess(t *testing.T) {
	...
	publishTests := []struct {
		...
		{
			name: "Should publish @productize/extension-sdk publicly",
			want: "npm publish --workspace @productize/extension-sdk --access public",
		},
		{
			name: "Should publish @productize/create-extension publicly",
			want: "npm publish --workspace @productize/create-extension --access public",
		},
	}
