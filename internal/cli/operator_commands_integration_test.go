...
func TestWorkspaceCommandsReflectDaemonRegistryAgainstRealDaemon(t *testing.T) {
	...
	for _, dir := range []string{workspaceA, workspaceB} {
		if err := os.MkdirAll(filepath.Join(dir, ".productize", "tasks"), 0o755); err != nil {
...
func TestWorkspaceCommandsIgnoreGlobalHomeMarkerForProjectsWithoutLocalWorkspace(t *testing.T) {
	...
	markerPath := filepath.Join(homeDir, ".productize")
...
func TestWorkspaceCommandsResolveRelativePathsAgainstRealDaemon(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(workspaceRoot, ".productize", "tasks"), 0o755); err != nil {
...
func TestSyncAndArchiveCommandsUseDaemonStateFromWorkspaceSubdirectory(t *testing.T) {
	...
	matches, err := filepath.Glob(filepath.Join(workspaceRoot, ".productize", "tasks", "_archived", "*-complete"))
	if err != nil {
	...
	if _, err := os.Stat(filepath.Join(workspaceRoot, ".productize", "tasks", "blocked")); err != nil {
...
func mustCLITestHomePaths(t *testing.T) productizeconfig.HomePaths {
	t.Helper()
...
func newShortCLITestHomeDir(t *testing.T) string {
	...
	homeDir, err := os.MkdirTemp(parent, "productize-cli-home-")
	if err != nil {
...
func openCLITestGlobalDB(t *testing.T, paths productizeconfig.HomePaths) *globaldb.GlobalDB {
	t.Helper()
...
func waitForCLITestDaemonState(t *testing.T, paths productizeconfig.HomePaths, want daemon.ReadyState) {
	t.Helper()
