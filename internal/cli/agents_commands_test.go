...
func TestExecCommandUnknownAgentReturnsActionableError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "productize agents list") {
...
func TestAgentsInspectInvalidAgentPrintsValidationBeforeNonZero(t *testing.T) {
  ...
  "mcpServers": {
	...
	if err != nil {
		t.Fatalf("expected agent inspection to succeed\nerror: %v\noutput:\n%s", err, output)
	}
	for _, want := range []string{
		"Agent: planner",
		"Status: valid",
...
func TestBuildInspectAgentReportAndListHelpers(t *testing.T) {
	...
	valid := reusableagents.ResolvedAgent{
		...
		Source: reusableagents.Source{
			...
			Dir:            "/workspace/.productize/agents/council",
			DefinitionPath: "/workspace/.productize/agents/council/AGENT.md",
			MCPConfigPath:  "/workspace/.productize/agents/council/mcp.json",
		},
		MCP: &reusableagents.MCPConfig{
			Path: "/workspace/.productize/agents/council/mcp.json",
	...
	invalid := reusableagents.Problem{
		...
		Source: reusableagents.Source{
			...
			Dir:            "/home/.productize/agents/planner",
			DefinitionPath: "/home/.productize/agents/planner/AGENT.md",
			MCPConfigPath:  "/home/.productize/agents/planner/mcp.json",
		},
		Err: fmt.Errorf(
			"%w: /home/.productize/agents/planner/mcp.json references %q",
...
func TestDecorateReusableAgentErrorAndHelperFormatting(t *testing.T) {
	...
	root := &cobra.Command{Use: "productize"}
	root.AddCommand(cmd)
	...
	if !strings.Contains(notFoundErr.Error(), "productize agents list") {
	...
	if !strings.Contains(invalidErr.Error(), "productize agents inspect planner") {
...
func TestIsReusableAgentValidationErrorCoversSupportedValidationKinds(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "reserved name", err: fmt.Errorf("%w: productize", reusableagents.ErrReservedAgentName), want: true},
		{
		...
		{name: "missing env", err: fmt.Errorf("%w: TOKEN", reusableagents.ErrMissingEnvironmentVariable), want: true},
		{
			...
			err:  fmt.Errorf("%w: productize", reusableagents.ErrReservedMCPServerName),
			want: true,
		},
