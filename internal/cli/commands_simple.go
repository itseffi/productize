...
func newMigrateCommand(dispatcher dispatcherProvider) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceUsage: true,
		Args:         cobra.NoArgs,
		Long: `Convert legacy XML-tagged workflow artifacts under .productize/tasks into Markdown frontmatter.

By default, the command scans the whole project workflow root recursively.`,
		Example: `  productize migrate
  productize migrate --dry-run
  productize migrate --name my-feature
  productize migrate --reviews-dir .productize/tasks/my-feature/reviews-001`,
		RunE: state.run,
	}

	cmd.Flags().StringVar(&state.rootDir, "root-dir", "", "Workflow root to scan (default: .productize/tasks)")
...
func newSyncCommand(dispatcher dispatcherProvider) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceUsage: true,
		Args:         cobra.NoArgs,
		Long: `Parse workflow artifacts under .productize/tasks and reconcile their
...
By default, the command scans the whole workflow root and syncs every active workflow.`,
		Example: `  productize sync
  productize sync --name my-feature
  productize sync --tasks-dir .productize/tasks/my-feature`,
		RunE: state.run,
	}
...
func newArchiveCommand(dispatcher dispatcherProvider) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceUsage: true,
		Args:         cobra.NoArgs,
		Long: `Archive fully completed workflows under .productize/tasks by moving them into
.productize/tasks/_archived/<timestamp-ms>-<shortid>-<slug>.
...
workflow state; workspace-wide archive requests skip ineligible workflows deterministically.`,
		Example: `  productize archive
  productize archive --name my-feature
  productize archive --tasks-dir .productize/tasks/my-feature`,
		RunE: state.run,
	}
