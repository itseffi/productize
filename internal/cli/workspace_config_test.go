...
`)
	...
	globalConfigPath := filepath.Join(homeDir, ".productize", "config.toml")
...
`)
	...
	extensionDir := filepath.Join(workspaceRoot, ".productize", "extensions", "provider-ext")
...
func TestNewFormInputsFromStatePreservesResolvedDefaults(t *testing.T) {
	...
	state := &commandState{
		workflowIdentity: workflowIdentity{
			name:     "demo",
			tasksDir: "/tmp/demo/.productize/tasks/demo",
		},
...
func TestBuildConfigMapsEmbeddedStateGroups(t *testing.T) {
	...
	state := &commandState{
		...
		workflowIdentity: workflowIdentity{
			...
			round:      4,
			nitpicks:   true,
			reviewsDir: "/workspace/.productize/tasks/demo/reviews-004",
