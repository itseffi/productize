...
func TestDaemonStartCommandDetachedReturnsReadyStatus(t *testing.T) {
	...
	readyClient := &stubDaemonCommandClient{
		target: apiclient.Target{SocketPath: "/tmp/productize-ready.sock"},
		status: apicore.DaemonStatus{
			PID:            4242,
...
func TestLaunchCLIDaemonProcessFailsWhenDaemonLogFileCannotBeOpened(t *testing.T) {
	...
	paths, err := productizeconfig.ResolveHomePathsFrom(t.TempDir())
	if err != nil {
		t.Fatalf("ResolveHomePathsFrom() error = %v", err)
	}
	paths.LogFile = paths.LogsDir

	err = launchCLIDaemonProcessWithExecutable(paths, filepath.Join(t.TempDir(), "unused-productize"))
	if err == nil {
...
func TestCLIDaemonBootstrapEnsureReusesHealthyDaemon(t *testing.T) {
	...
	bootstrap := cliDaemonBootstrap{
		...
		readInfo: func(path string) (daemon.Info, error) {
			if path != "/tmp/productize-home/daemon.json" {
	...
	if len(newClientTargets) != 1 || newClientTargets[0].SocketPath != "/tmp/productize-ready.sock" {
...
func TestCLIDaemonBootstrapProbeReportsNotReadyHealth(t *testing.T) {
	...
	bootstrap := cliDaemonBootstrap{
		readInfo: func(string) (daemon.Info, error) {
			return daemon.Info{
				PID:        1234,
				SocketPath: "/tmp/productize-health.sock",
	...
	_, err := bootstrap.probe(context.Background(), "/tmp/productize-home/daemon.json")
	if err == nil {
		t.Fatal("expected probe failure for not-ready daemon health")
	}
	if got := err.Error(); !containsAll(
		got,
		"probe daemon health via unix:///tmp/productize-health.sock",
...
func TestCLIDaemonBootstrapProbeWrapsReadInfoAndClientErrors(t *testing.T) {
	...
	_, err := readInfoErrBootstrap.probe(context.Background(), "/tmp/productize-home/daemon.json")
	...
	_, err = newClientErrBootstrap.probe(context.Background(), "/tmp/productize-home/daemon.json")
...
func TestCLIDaemonBootstrapEnsureRepairsStaleTransportAfterLaunch(t *testing.T) {
	...
	staleClient := &stubDaemonCommandClient{
		target:    apiclient.Target{SocketPath: "/tmp/productize-stale.sock"},
		healthErr: errors.New("dial unix /tmp/productize-stale.sock: connect: no such file or directory"),
	}
	readyClient := &stubDaemonCommandClient{
		target: apiclient.Target{SocketPath: "/tmp/productize-stale.sock"},
		health: apicore.DaemonHealth{Ready: true},
	...
	bootstrap := cliDaemonBootstrap{
		...
		readInfo: func(string) (daemon.Info, error) {
			return daemon.Info{
				PID:        1234,
				SocketPath: "/tmp/productize-stale.sock",
...
func TestSyncCommandUsesDaemonBackedRequestAndJSONOutput(t *testing.T) {
	...
	workflowDir := filepath.Join(workspaceRoot, ".productize", "tasks", "demo")
	...
	client := &stubDaemonCommandClient{
		...
		syncResult: apicore.SyncResult{
			WorkspaceID:       "ws-123",
			WorkflowSlug:      "demo",
			Target:            filepath.Join(workspaceRoot, ".productize", "tasks", "demo"),
			WorkflowsScanned:  1,
			TaskItemsUpserted: 3,
		},
	}
...
func TestArchiveCommandWorkspaceWideSkipsConflictsDeterministically(t *testing.T) {
	...
	for _, slug := range []string{"alpha", "beta"} {
		if err := os.MkdirAll(filepath.Join(workspaceRoot, ".productize", "tasks", slug), 0o755); err != nil {
