...
import (
	...
	"github.com/itseffi/productize/internal/core/reviews"
	coreRun "github.com/itseffi/productize/internal/core/run"
...
func TestRunsPurgeCommandRemovesTerminalRunArtifactsOldestFirst(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(workspaceRoot, ".productize"), 0o755); err != nil {
...
`)
	...
	staleClient := &stubDaemonCommandClient{
		...
		healthErr: errors.New("dial unix /tmp/productize-daemon.sock: connect: no such file or directory"),
	}
...
func TestTasksRunCommandBootstrapFailureReturnsStableExitCode(t *testing.T) {
	...
	installTestCLIDaemonBootstrap(t, cliDaemonBootstrap{
		...
		newClient: func(apiclient.Target) (daemonCommandClient, error) {
			return &stubDaemonCommandClient{
				target:    apiclient.Target{SocketPath: "/tmp/productize-daemon.sock"},
				healthErr: errors.New("dial unix /tmp/productize-daemon.sock: connect: connection refused"),
			}, nil
		},
		launch:         func(productizeconfig.HomePaths) error { return nil },
		sleep:          func(time.Duration) {},
		now:            nextNow,
	...
	if !containsAll(stderr, "wait for daemon readiness", "probe daemon health via unix:///tmp/productize-daemon.sock") {
...
func TestLegacyCommandsAreRemoved(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			expected := fmt.Sprintf("unknown command %q for %q", tc.command, "productize")
...
func TestReviewsFixCommandExecuteDryRunPersistsKernelArtifacts(t *testing.T) {
	workspaceRoot := t.TempDir()
	reviewDir := filepath.Join(workspaceRoot, ".productize", "tasks", "demo", "reviews-001")
	...
	for _, want := range []string{"`fix-reviews`", "issue_001.md", "internal/app/service.go"} {
...
func TestTaskAndReviewCommandsExecuteDryRunAgainstTempNodeWorkspace(t *testing.T) {
	...
	for _, want := range []string{"`fix-reviews`", "issue_001.md", "src/server.js"} {
...
func latestRunDirForCLI(t *testing.T, _ string) string {
	...
	homePaths, err := productizeconfig.ResolveHomePaths()
	if err != nil {
...
func assertNoRunArtifactsForCLI(t *testing.T, workspaceRoot string) {
	...
	if _, err := os.Stat(filepath.Join(workspaceRoot, ".productize", "runs")); !os.IsNotExist(err) {
...
func writeCLINodeWorkflowFixture(t *testing.T, workspaceRoot string, slug string) {
	...
	workflowDir := filepath.Join(workspaceRoot, ".productize", "tasks", slug)
...
## Executive Summary
This fixture adds a small readiness endpoint to a minimal Node.js API so daemon-backed Productize task and review flows have realistic files to inspect. The goal is deterministic operator-flow validation, not production feature depth.
...
- The task is independently executable through Productize
`,
	}
...
func installWorkspaceMockExtensionForCLI(
	...
	extensionDir := filepath.Join(workspaceRoot, ".productize", "extensions", "mock-ext")
...
env = { PRODUCTIZE_MOCK_RECORD_PATH = %q, PRODUCTIZE_MOCK_MODE = %q, PRODUCTIZE_MOCK_WORKFLOW = %q }
