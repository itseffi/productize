...
func committedACPFixtureDir(repoRoot string) (string, error) {
	activePath := filepath.Join(repoRoot, ".productize", "tasks", "acp-integration")
	if info, err := os.Stat(activePath); err == nil && info.IsDir() {
		return activePath, nil
	}

	matches, err := filepath.Glob(filepath.Join(repoRoot, ".productize", "tasks", "_archived", "*-acp-integration"))
	if err != nil {
		return "", err
	}
