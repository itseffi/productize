...
Workspace agents live under .productize/agents/<name>/ and override same-name agents from
~/.productize/agents/<name>/ as a whole directory.`,
...
func newMCPServeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "mcp-serve",
		Short:        "Serve the reserved Productize MCP server over stdio",
		SilenceUsage: true,
		Args:         cobra.NoArgs,
		Hidden:       true,
		RunE:         state.run,
...
func buildInspectAgentReport(catalog reusableagents.Catalog, name string) (inspectAgentReport, error) {
	...
	if len(available) == 0 {
		return inspectAgentReport{}, fmt.Errorf(
			"reusable agent %q not found; no reusable agents are currently available. Run `productize agents list` for details",
			normalized,
		)
	}
	return inspectAgentReport{}, fmt.Errorf(
		"reusable agent %q not found; available agents: %s. Run `productize agents list` for details",
		normalized,
...
func writeAgentsListText(out io.Writer, catalog reusableagents.Catalog) error {
	if len(catalog.Agents) == 0 {
		if _, err := fmt.Fprintln(
			out,
			"no reusable agents found under .productize/agents or ~/.productize/agents",
		); err != nil {
			return err
		}
	} else {
	...
	if _, err := fmt.Fprintln(
		out,
		"Run `productize agents inspect <name>` to inspect one definition in detail.",
	); err != nil {
		return err
	}
	return nil
...
func writeInspectAgentText(out io.Writer, report inspectAgentReport) error {
	...
	_, err := fmt.Fprintf(
		out,
		"Next step: fix the agent definition and rerun `productize agents inspect %s`\n",
		report.Name,
	)
	return err
}
