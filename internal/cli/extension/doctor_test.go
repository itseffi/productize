...
func TestDoctorWarnsOnExtensionReusableAgentDriftInProjectScope(t *testing.T) {
	...
	writeTestFile(
		t,
		filepath.Join(deps.workspaceRoot, ".productize", "agents", "product-scout", "AGENT.md"),
		"drifted\n",
	)
...
func TestDoctorReturnsErrorForUnsupportedMinProductizeVersion(t *testing.T) {
	deps := newTestDeps(t)
	withProductizeVersion(t, "1.0.0")

	manifest := manifestFixture("future-ext")
	manifest.Extension.MinProductizeVersion = "9.0.0"
	...
	if !strings.Contains(output, "requires Productize 9.0.0 or newer") {
...
func withProductizeVersion(t *testing.T, value string) {
	t.Helper()
