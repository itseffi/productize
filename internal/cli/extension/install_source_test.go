...
func TestResolveInstallSourceWithFetcherGitHub(t *testing.T) {
	...
	archive := buildTarGz(t, []archiveEntry{
		{name: "productize-v1.2.3/", typeflag: tar.TypeDir},
		{
			name:     "productize-v1.2.3/.productize/tasks/_archived/20260410-192523-refac/_techspec.md",
...
`,
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got, want := r.URL.Path, "/itseffi/productize/tar.gz/v1.2.3"; got != want {
	...
	if got.InstallOrigin.Repository != "itseffi/productize" || got.InstallOrigin.Ref != "v1.2.3" {
...
func TestExtractTarGzArchiveIgnoresSymlinkOutsideIncludedSubdir(t *testing.T) {
	...
	archive := buildTarGz(t, []archiveEntry{
		{name: "repo/", typeflag: tar.TypeDir},
		{
			name:     "repo/.productize/tasks/_archived/20260410-192523-refac/_techspec.md",
			typeflag: tar.TypeSymlink,
			linkname: "20260406-summary.md",
		},
	...
	if _, err := os.Lstat(filepath.Join(extractedRoot, ".productize")); !os.IsNotExist(err) {
