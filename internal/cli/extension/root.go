...
// NewExtCommand constructs the builtin `productize ext` command group.
...
func newExtCommandWithDeps(_ *kernel.Dispatcher, deps commandDeps) *cobra.Command {
	cmd := &cobra.Command{
		Use:          "ext",
		Short:        "Manage discovered Productize extensions",
		SilenceUsage: true,
		Long: `Inspect and manage bundled, user, and workspace Productize extensions.
