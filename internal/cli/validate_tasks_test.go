...
func TestValidateTasksCommandMissingDir(t *testing.T) {
	...
	missingDir := filepath.Join(workspaceRoot, ".productize", "tasks", "missing")
...
func TestValidateTasksCommandResolvesRelativeTasksDirFromWorkspaceRoot(t *testing.T) {
	...
	stdout, stderr, exitCode := runValidateTasksCommand(
		...
		"validate",
		"--tasks-dir",
		filepath.Join(".productize", "tasks", "demo"),
	)
	if exitCode != 0 {
		t.Fatalf("expected exit code 0, got %d\nstdout:\n%s\nstderr:\n%s", exitCode, stdout, stderr)
	}
...
func TestResolveTaskWorkflowDir(t *testing.T) {
	...
	want := filepath.Join(root, ".productize", "tasks", "demo")
	if got != want {
...
func validateTasksBinary(t *testing.T) string {
	...
	validateTasksBinaryOnce.Do(func() {
		...
		buildDir, err := os.MkdirTemp("", "productize-validate-tasks-*")
		if err != nil {
			validateTasksBinaryErr = err
			return
		}

		validateTasksBinaryPath = filepath.Join(buildDir, "productize")
		cmd := exec.CommandContext(
			context.Background(),
			"go",
			"build",
			"-o",
			validateTasksBinaryPath,
			"./cmd/productize",
		)
		cmd.Dir = repoRoot
		...
		if err != nil {
			validateTasksBinaryErr = fmt.Errorf("build productize binary: %w\n%s", err, output)
		}
	})
...
func makeValidateTasksWorkspace(t *testing.T, name string) (string, string) {
	...
	root := t.TempDir()
	tasksDir := filepath.Join(root, ".productize", "tasks", name)
