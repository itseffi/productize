...
import (
	...
	"github.com/itseffi/productize/internal/core/modelprovider"
...
func TestPrepareAndRunInstallsEnabledExtensionSkillPackDuringPreflight(t *testing.T) {
	...
	extensionDir := filepath.Join(workspaceRoot, ".productize", "extensions", "skills-ext")
...
func TestFetchReviewsUsesEnabledExtensionProviderOverlay(t *testing.T) {
	...
	extensionDir := filepath.Join(workspaceRoot, ".productize", "extensions", "review-ext")
	writeBootstrapManifestJSON(t, extensionDir, manifest)
	...
	state.fetchReviewsFn = func(_ context.Context, cfg core.Config) (*core.FetchResult, error) {
		...
		return &core.FetchResult{
			...
			PR:         cfg.PR,
			Round:      1,
			ReviewsDir: filepath.Join(workspaceRoot, ".productize", "tasks", cfg.Name, "reviews-001"),
			Total:      1,
		}, nil
	}
...
func TestPrepareAndRunAcceptsEnabledExtensionACPRuntimeOverlay(t *testing.T) {
	...
	extensionDir := filepath.Join(workspaceRoot, ".productize", "extensions", "runtime-ext")
	writeBootstrapManifestJSON(t, extensionDir, manifest)
...
func TestBootstrapDeclarativeAssetsActivatesModelAliasOverlay(t *testing.T) {
	...
	extensionDir := filepath.Join(workspaceRoot, ".productize", "extensions", "model-ext")
	writeBootstrapManifestJSON(t, extensionDir, manifest)
