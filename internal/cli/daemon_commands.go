...
import (
	...
	daemonlogger "github.com/itseffi/productize/internal/logger"
...
var (
	resolveCLIDaemonHomePaths  = productizeconfig.ResolveHomePaths
...
func defaultLaunchCLIDaemonProcess(paths productizeconfig.HomePaths) error {
	...
	if strings.HasSuffix(base, ".test") || strings.HasSuffix(base, ".test.exe") {
		return "", errors.New(
			"daemon auto-start cannot relaunch a Go test binary; " +
				"install a daemon bootstrap stub or use a real productize executable",
		)
	}
...
is running, and then sends the workflow request over the daemon transport.`,
		Example: `  productize tasks run my-feature
  productize tasks run my-feature --stream
  productize tasks run my-feature --detach
  productize tasks run --name my-feature --dry-run`,
