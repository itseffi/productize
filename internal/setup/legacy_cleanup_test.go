...
func TestCleanupLegacyTransferredAssetsRemovesProjectScopeInstalls(t *testing.T) {
	...
	legacyPaths := []string{
		filepath.Join(projectDir, ".agents", "skills", "idea-factory", "SKILL.md"),
		filepath.Join(projectDir, ".claude", "skills", "idea-factory", "SKILL.md"),
		filepath.Join(projectDir, ".productize", "agents", "architect-advisor", "AGENT.md"),
	}
	...
	unrelatedPaths := []string{
		filepath.Join(projectDir, ".agents", "skills", "create-prd", "SKILL.md"),
		filepath.Join(projectDir, ".productize", "agents", "custom-agent", "AGENT.md"),
	}
...
func TestCleanupLegacyTransferredAssetsRemovesGlobalScopeInstalls(t *testing.T) {
	...
	legacyPaths := []string{
		filepath.Join(homeDir, ".agents", "skills", "idea-factory", "SKILL.md"),
		filepath.Join(homeDir, ".claude", "skills", "idea-factory", "SKILL.md"),
		filepath.Join(homeDir, ".productize", "agents", "product-mind", "AGENT.md"),
	}
	for i := range legacyPaths {
		writeLegacyCleanupFixture(t, legacyPaths[i])
	}
