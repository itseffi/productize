...
func resolveEffectiveSkills(bundled []Skill, extension []Skill) ([]Skill, []CatalogConflict) {
	...
	// Preserve precedence order: bundled/core skills first, then extension
	// skills in source precedence order. Sorting by name here can incorrectly
	// rank an extension skill ahead of a core skill whose name sorts later.
...
func resolveEffectiveReusableAgents(
	...
	// Preserve precedence order: bundled reusable agents first, then extension
	// agents in source precedence order (see resolveEffectiveSkills).
	return effective, conflicts
}
