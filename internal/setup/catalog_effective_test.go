...
func TestBuildEffectiveCatalogPrefersCoreAndHigherPrecedenceExtensions(t *testing.T) {
	...
	catalog := BuildEffectiveCatalog(
		[]Skill{
			{Name: "productize", Origin: AssetOriginBundled},
		},
		[]Skill{
			{
				Name:            "productize",
	...
	if got := skillNames(catalog.Skills); len(got) != 2 || got[0] != "productize" || got[1] != "idea-pack" {
...
func TestInstallSelectedSkillsCopiesBundledAndExtensionSkills(t *testing.T) {
	...
	assertFileExists(t, filepath.Join(projectDir, ".agents", "skills", "productize", "SKILL.md"))
