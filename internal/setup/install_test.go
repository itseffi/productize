	...
	tests := []struct {
	...
	}{
		{
			...
			wantRoot: func(projectDir, _ string) string {
				return filepath.Join(projectDir, ".productize", "agents")
			},
		},
		{
			name:   "Global scope",
			...
			wantRoot: func(_ string, homeDir string) string {
				return filepath.Join(homeDir, ".productize", "agents")
			},
		},
	}
...
func TestInstallReusableAgentsPreservesExistingInstallWhenCopyFails(t *testing.T) {
	projectDir := t.TempDir()
	homeDir := t.TempDir()
	existingAgentDir := filepath.Join(homeDir, ".productize", "agents", "architect-advisor")
	...
	stagedPaths, err := filepath.Glob(filepath.Join(homeDir, ".productize", "agents", "architect-advisor.tmp-*"))
	if err != nil {
...
func TestInstallReusableAgentsInProjectScopePreservesGlobalInstall(t *testing.T) {
	...
	projectDir := t.TempDir()
	homeDir := t.TempDir()
	globalAgentDir := filepath.Join(homeDir, ".productize", "agents", "architect-advisor")
	...
	projectAgentDir := filepath.Join(projectDir, ".productize", "agents", "architect-advisor")
