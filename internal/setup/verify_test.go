...
func TestVerifyProjectInstallMatchesBundledSkills(t *testing.T) {
	...
	_, err := Install(InstallConfig{
		...
		SkillNames: []string{"create-prd", "final-verify"},
...
func TestVerifyPrefersProjectScopeOverGlobalWhenProjectInstallIsPartial(t *testing.T) {
	...
	if got := result.MissingSkillNames(); !reflect.DeepEqual(got, []string{"final-verify"}) {
		t.Fatalf("unexpected missing skills\nwant: %#v\ngot:  %#v", []string{"final-verify"}, got)
	}
}
...
func TestVerifyReportsChangedFilesAsDrift(t *testing.T) {
	...
	skillPath := filepath.Join(projectDir, ".claude", "skills", "create-prd", "SKILL.md")
	...
	if got := result.DriftedSkillNames(); !reflect.DeepEqual(got, []string{"create-prd"}) {
		t.Fatalf("unexpected drifted skills\nwant: %#v\ngot:  %#v", []string{"create-prd"}, got)
	}
...
func TestVerifyReportsExtraFilesAsDrift(t *testing.T) {
	...
	extraPath := filepath.Join(projectDir, ".claude", "skills", "create-prd", "notes.txt")
...
func TestVerifyReusableAgentsReportsProjectDriftWhenProjectOverridesGlobal(t *testing.T) {
	...
	projectAgentPath := filepath.Join(projectDir, ".productize", "agents", "architect-advisor", "AGENT.md")
...
func TestVerifyReusableAgentsUsesProjectOverridesAndGlobalFallbackPerAgent(t *testing.T) {
	...
	if want := filepath.Join(projectDir, ".productize", "agents", "architect-advisor"); architect.TargetPath != want {
	...
	if want := filepath.Join(homeDir, ".productize", "agents", "product-mind"); product.TargetPath != want {
