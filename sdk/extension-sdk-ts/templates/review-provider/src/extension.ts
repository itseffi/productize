...
import { CAPABILITIES, Extension } from "@productize/extension-sdk";
import type { FetchRequest, ResolveIssuesRequest, ReviewItem } from "@productize/extension-sdk";
...
export function createReviewProviderExtension(
  ...
  const mode = process.env.PRODUCTIZE_TS_REVIEW_MODE?.trim() ?? "";
...
function record(kind: string, payload: Record<string, unknown>): void {
  const path = process.env.PRODUCTIZE_TS_RECORD_PATH?.trim();
