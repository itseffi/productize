...
const CREATE_EXTENSION_PACKAGE_NAME = "@productize/create-extension";
const PRODUCTIZE_MODULE_PATH = "github.com/itseffi/productize";
...
export function printHelp(): string {
  return [
    ...
    "  --go-sdk-replace <dir> Local Productize repo path to use via go.mod replace",
...
async function buildTokenMap(name: string, sdkSpec?: string): Promise<Record<string, string>> {
  ...
  return {
    ...
    __PRODUCTIZE_MIN_VERSION__: metadata.version,
    __PRODUCTIZE_EXTENSION_SDK_SPEC__:
      sdkSpec ?? process.env.PRODUCTIZE_EXTENSION_SDK_SPEC ?? metadata.version,
...
min_productize_version = "${metadata.version}"
...
Scaffolded Productize ${options.template} extension in Go.
`;
...
async function installGoSDK(targetDir: string, options: CreateExtensionOptions): Promise<void> {
  const replacePath = resolveOptionalPath(
    options.goSDKReplace ?? process.env.PRODUCTIZE_GO_SDK_REPLACE ?? ""
  );
  if (replacePath !== "") {
    await runCommand(
      "go",
      ["mod", "edit", `-replace=${PRODUCTIZE_MODULE_PATH}=${replacePath}`],
      targetDir
    );
    return;
  }
  ...
  for (const ref of preferredGoSDKRefs(options.goSDKRef, metadata.version)) {
    try {
      await runCommand("go", ["get", `${PRODUCTIZE_MODULE_PATH}/sdk/extension@${ref}`], targetDir);
      return;
    } catch (error) {
      lastError = error;
    }
...
function preferredGoSDKRefs(explicitRef: string | undefined, packageVersion: string): string[] {
  const refs = [
    explicitRef,
    process.env.PRODUCTIZE_GO_SDK_REF,
