...
describe("@productize/create-extension", () => {
  ...
  it("emits a working CLI entrypoint", async () => {
    const root = await mkdtemp(join(tmpdir(), "productize-create-extension-cli-"));
    ...
    const result = await execFileAsync("node", [createExtensionCLI, "cli-ext", "--skip-install"], {
      ...
      env: {
        ...process.env,
        PRODUCTIZE_EXTENSION_SDK_SPEC: localSDKSpec,
      },
    });
  ...
  it("copies the lifecycle observer template into a buildable project", async () => {
    const root = await mkdtemp(join(tmpdir(), "productize-create-extension-"));
    await buildLocalPackages();
  ...
  it("copies the review provider template into a buildable project", async () => {
    const root = await mkdtemp(join(tmpdir(), "productize-create-extension-review-"));
    await buildLocalPackages();
  ...
  it("scaffolds a Go project against the local repository when requested", async () => {
    const root = await mkdtemp(join(tmpdir(), "productize-create-extension-go-"));
    await buildLocalPackages();
    ...
    expect(goMod).toContain("replace github.com/itseffi/productize =>");
