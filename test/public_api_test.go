...
import (
	...
	"github.com/itseffi/productize/pkg/productize/runs/layout"
)

func TestPrepareAndRunExposePublicAPI(t *testing.T) {
	workspaceRoot := t.TempDir()
	t.Setenv("HOME", filepath.Join(t.TempDir(), "home"))
	tasksDir := filepath.Join(workspaceRoot, ".productize", "tasks", "demo")
...
`
	...
	prep, err := productize.Prepare(context.Background(), cfg)
	if err != nil {
	...
	if err := productize.Run(context.Background(), cfg); err != nil {
		t.Fatalf("run: %v", err)
	}
}

func TestNewCommandUsesProductizeRootCommand(t *testing.T) {
	t.Parallel()

	cmd := productize.NewCommand()
	if cmd == nil {
		t.Fatal("expected command")
	}
	if cmd.Use != "productize" {
		t.Fatalf("expected use productize, got %q", cmd.Use)
	}
}
...
func TestMigrateExposePublicAPI(t *testing.T) {
	...
	workflowDir := filepath.Join(tmpDir, ".productize", "tasks", "demo")
	...
	result, err := productize.Migrate(context.Background(), productize.MigrationConfig{DryRun: true})
	if err != nil {
...
func TestSyncExposePublicAPI(t *testing.T) {
	...
	result, err := productize.Sync(context.Background(), productize.SyncConfig{TasksDir: workflowDir})
	if err != nil {
		t.Fatalf("sync: %v", err)
	}
	if result == nil {
...
func TestArchiveExposePublicAPI(t *testing.T) {
	...
	if _, err := productize.Sync(context.Background(), productize.SyncConfig{TasksDir: workflowDir}); err != nil {
		t.Fatalf("sync before archive: %v", err)
	}

	result, err := productize.Archive(context.Background(), productize.ArchiveConfig{TasksDir: workflowDir})
	if err != nil {
