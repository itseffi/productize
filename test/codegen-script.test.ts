...
function createCodegenFixture(initialOutput: string) {
  const root = mkdtempSync(path.join(tmpdir(), "productize-codegen-script-"));
  ...
  const sourcePath = path.join(root, "openapi/productize-daemon.json");
  const targetPath = path.join(root, "web/src/generated/productize-openapi.d.ts");
