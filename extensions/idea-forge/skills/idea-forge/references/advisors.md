# Advisors of Advisors Reference

Run the advisors as a real embedded subagent workflow, not as inline roleplay. The facilitator remains inside `idea-forge`, but every advisor is dispatched through the host-owned `run_agent` tool using canonical reusable-agent ids.

## Runtime Contract

- The standard advisors roster is provisioned by `productize setup` under `~/.productize/agents/<name>/`.
- A workspace may override any advisor by defining `.productize/agents/<name>/`.
...
`idea-forge` always runs advisors in embedded mode:
