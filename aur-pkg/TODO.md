# TODO: Finalizing AUR Automation for Productize
...
- **Username:** `productize` (recommended) or your personal username.
...
1. **Generate a new keypair** (no passphrase):
   ```bash
   ssh-keygen -t ed25519 -f ~/.ssh/aur_productize -N ""
   ```
2. **Add the Public Key to AUR:**
   - Copy the content of `~/.ssh/aur_productize.pub`.
   - Log in to AUR -> My Account -> SSH Public Key -> Paste the key and Save.

3. **Add the Private Key to GitHub Secrets:**
   - Copy the content of the private key: `cat ~/.ssh/aur_productize`.
...
git clone ssh://aur@aur.archlinux.org/productize-bin.git
cd productize-bin

# Copy the PKGBUILD from this PR's aur-pkg directory
cp /path/to/productize/aur-pkg/PKGBUILD .
...
git commit -m "Initial commit for productize-bin"
git push origin master
```

## 4. Update .goreleaser.yml (If needed)

Ensure the `release.github.owner` in `.goreleaser.yml` matches the upstream organization (`productize`).
