...
Complete reference for `.productize/config.toml` workspace configuration.

## File Location

Place the configuration file at `.productize/config.toml` in the workspace root. CLI flags always override config values.
...
Options specific to `productize tasks run`.

| Field | Type | Description |
| --- | --- | --- |
| `include_completed` | bool | Include tasks already marked as completed |
| `task_runtime_rules` | `array<table>` | Type-scoped runtime overrides applied after `[defaults]` for `productize tasks run` |
...
Options specific to `productize reviews fix`.
...
Options specific to `productize reviews fetch`.
...
Options specific to `productize reviews watch`. Watch-specific loop values come from this section; child review
...
```bash
productize reviews watch --provider coderabbit --pr 123
productize reviews watch my-workflow --provider coderabbit --pr 123 --max-rounds 4
productize reviews watch my-workflow --provider coderabbit --pr 123 --auto-push --push-remote origin --push-branch feature/reviews
```
...
Options specific to `productize exec`. Inherits all `[defaults]` fields plus:
...
Optional audio notifications that play when a run reaches a terminal state. Applies to both `productize tasks run` and `productize exec`. Disabled by default — setting any field without `enabled = true` is a no-op.
