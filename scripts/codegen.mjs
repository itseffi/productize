 ...
 * Regenerates `web/src/generated/productize-openapi.d.ts` from
 * `openapi/productize-daemon.json` via `openapi-typescript`.
 *
...
const source = resolve(repoRoot, "openapi/productize-daemon.json");
const target = resolve(repoRoot, "web/src/generated/productize-openapi.d.ts");
...
const workDir = mkdtempSync(join(tmpdir(), "productize-codegen-"));
try {
  const candidate = join(workDir, "productize-openapi.d.ts");
  run(candidate);
  ...
  if (next !== current) {
    console.error(
      [
        "codegen-check: web/src/generated/productize-openapi.d.ts is out of date.",
